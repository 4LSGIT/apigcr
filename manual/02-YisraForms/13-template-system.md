...
- **Granularity is authoring.** One card = one section. For the one-question-per-card feel, make one section per question (the field's label styles as the card heading when the section has no title and exactly one *base* question). A question carrying `showWhen` counts as a **follow-up**, not a second question — so a list plus its own conditional "please specify" box keeps the single-question heading. Multi-field sections give grouped cards.
- **Navigation.** Next validates the current card only (the real `validate()` run under a narrowed rule set — same messages, same masks, same `requiredWhen` semantics, and conditions may reference fields on other cards). Back never validates. Dots: current = ring, visited-valid = green, visited-with-errors = red (hover shows the card title, or the question's label on an untitled card); visited dots are clickable, unvisited are not. Leaving a card by any route silently records its state for the dot color.
- **Auto-advance.** A card with exactly one *visible* question advances ~300 ms after a **terminal gesture** on it — one interaction that completes the answer — provided the card's validation passes. That means `radio`, `select`, and `checkbox` (on the transition to checked only; clearing a select or unchecking is un-answering and never advances). Deliberately excluded: `text`/`textarea`/`number`/`tags`, whose `change` fires on **blur** — including the blur caused by clicking Next; `date`/`datetime`, where a mistyped year commits a perfectly valid `change` and the submitter is a card away before noticing; and `checkgroup`, which is multi-select by definition. If the selection reveals a follow-up field on the *same* card, the advance is cancelled rather than skipping past it. On the last card an auto-advance enters the review page.
- **Conditional cards.** A section-level `showWhen` collapses the whole card out of the sequence — the dot disappears and Next skips it. A card whose questions are *all* condition-hidden is skipped the same way, so promoting a field-level `showWhen` onto its own card behaves correctly either way (a `content`-only card, having no questions, is never swept up by this). Row/field `showWhen` works inside a card as everywhere.
- **Review page.** The last card's Next reads "Review and Submit" and runs the **full, unscoped** validation — a required field on any card (visited or not) blocks entry with that card revealed and scrolled, mirroring tab-reveal. The review lists every visible answer, numbered 1..N across the whole form (option **labels**, checkbox Yes/No, blank = —, `hidden` and display-only fields skipped, repeater items enumerated), each row carrying its own Edit link back to its card, and titled cards keeping their heading; Submit is the standard `#saveBtn`, revealed only here, and a failed Submit likewise reveals the offending card.
...
`{ "name": "firm_logo", "type": "content", "src": "https://…/logo.png", "alt": "…", "maxWidth": 400, "align": "center", "href": "https://…", "text": "Optional caption" }` renders an image and/or a text block inside a row — a logo, an illustration, or standalone display copy. At least one of `src` / `text` is required; `src` and `href` must be https (≤ 2000 chars each); `maxWidth` is a positive-int pixel cap (the image is always additionally capped at 100% of its column); `align` is `left` (default) / `center` / `right`; `text` (≤ 2000 chars) prints as a caption under the image, or as the block's only content when there is no image. Real newlines in `text` render as line and paragraph breaks (the caption is styled `white-space: pre-wrap`) — that is the *only* formatting available and deliberately so: display copy reaches the DOM through `textContent` with no markup path, which is the entire reason `content` is safe on public forms. There is no bold, no links inside the text, and no HTML or markdown; a rich-text type would have to either be refused externally — useless for the public intake that motivates it — or ship a sanitizer on an unauthenticated endpoint, which is its own security review, not an addition to this field.
