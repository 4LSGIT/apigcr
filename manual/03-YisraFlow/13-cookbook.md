...
Related: `sequence_templates.condition` is a **write-once legacy column** (create-time value; read only as a v0 fallback by `ensureDraft`, duplicate, the template-PUT no-op guard, and both GET overlays; never updated after the first fork). The truth is `sequence_template_versions.template_condition` for the relevant version. See chapter 16.
