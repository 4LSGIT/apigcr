...
- **Custom fields ride the envelope as their own `cf_*` keys** — `data.cf_referral`, `changes.cf_referral.to` — exactly like a real column, because each active one IS a column on the table. The raw `custom` bag never travels, on purpose: it is stripped by name, so `data.custom` is always absent and a rule must name the field. `data.cf_x` is the value the record now holds on both entities; `changes.cf_x.to` is the value just written and is the safer thing to match when you mean "was set to". See [Custom Fields](../05-Subsystems/13-custom-fields.md).
