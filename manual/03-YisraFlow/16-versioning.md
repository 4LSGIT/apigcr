...
2. **Retired drafts accumulate step rows.** No reaper, no cap — by design. `GET :id/versions` reports `retired_versions` / `retired_step_rows` so the growth is observable. Negligible today (a few hundred step rows total); revisit if version churn gets heavy.
