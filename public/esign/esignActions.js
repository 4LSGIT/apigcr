...
// (signing_request_sources), and POST /api/esign/:id/resend falls back to it
// when no file is attached. So:
//   bounced          → same-row resend: editable recipient emails + an
//                      OPTIONAL file input when the detail says has_source
//                      (attach only to REPLACE the document); REQUIRED for
//                      pre-2E rows where nothing was stored.
//   terminal         → duplicate-as-new: same optional/required file logic.
//     (declined/       If the original was template-made (row.template_id),
//      recalled/       the dialog still points at the send form prefilled —
//      expired)        re-manufacturing with current case data is usually
//                      better than resending stale bytes.
//   anything else    → not offered (recall first; that must be deliberate).
// The dialog needs `has_source`, which rides GET /api/esign/:id (the detail),
// not the list rows — callers pass the detail's request object.
// ─────────────────────────────────────────────────────────────
...
if (typeof window !== 'undefined') (function () {
  ...
  async function esignDetailDialog(id, onChange) {
    ...
    _swal().fire({
      ...
      html:
        ...
        meta('Sent document', r.has_source
          ? '<a href="#" onclick="esignDownloadSource(' + Number(id) + ');return false">' +
            '<i class="fa-solid fa-file-arrow-down"></i> Download (unsigned copy)</a>'
          : '') +
        '</table>' +
  ...
  // ── SOURCE DOWNLOAD (2E) ───────────────────────────────────
  // The stored UNSIGNED source of a send. Authed binary GET → blob →
  // object-URL click — the pattern for Bearer-gated files (no plain <a href>
  // because the route needs the Authorization header).
  async function esignDownloadSource(id) {
    try {
      var out = await esignFetchBinary('/api/esign/' + id + '/source');
      var url = URL.createObjectURL(out.blob);
      var a = document.createElement('a');
      a.href = url;
      a.download = 'signing-request-' + id + ' (unsigned).pdf';
      document.body.appendChild(a);
      a.click();
      a.remove();
      setTimeout(function () { URL.revokeObjectURL(url); }, 10000);
    } catch (err) {
      _toast('error', 'Download failed', err.message);
    }
  }
  ...
  async function esignResendDialog(row, onChange) {
    // The list shape (_shapeForList) carries neither has_source nor
    // template_id — both drive this dialog's choices — so hydrate from the
    // detail when handed a list row. (This also fixes a latent gap: the
    // "resend from the template instead" steering below keyed on template_id,
    // which a list row never had.)
    if (row.has_source === undefined) {
      try {
        var d = await esignApi('/api/esign/' + row.id, 'GET');
        row = d.request || row;
      } catch (err) {
        _toast('error', 'Error loading request', err.message);
        return;
    ...
    _swal().fire({
      ...
      preConfirm: async function () {
        ...
        if (!hasFile && !row.has_source) {
          _swal().showValidationMessage('A PDF is required to re-send this request.');
          return false;
        }
        var fd = new FormData();
  ...
  window.esignDownloadSource  = esignDownloadSource;
