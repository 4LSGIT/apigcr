...
async function _scanEmailIngest(db, wm) {
  ...
  // (ref/migrations/2026-08-19_ingest_action_failure_count.sql). Using it
  // instead of a JSON_SEARCH predicate keeps ONE definition of "a failed
  // outcome" (the column expression) and lets the optimizer use
  // idx_action_failures — observed as an index_merge sort_union with
  // idx_status_created on a cold/backlogged watermark, or a PRIMARY range when
  // `id > ?` is already tiny. _failedOutcomes() below still parses metadata for
  // the alert BODY; only the row-selection predicate keys off the column.
  const [rows] = await db.query(
