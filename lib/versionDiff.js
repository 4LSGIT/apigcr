...
//              a loop with no pause and no foreach — see findPauseFreeCycles)
...
// ── findPauseFreeCycles(draftSteps, branchTargetParams) ────────────────────
// Added 2026-09-22 after the wf27 v6 runaway: a provisioning script appended
// a second copy of an 8-step block, so step 43 jumped back to 36 → 39
// (create_task) → 40…43 → 36 with nothing in the cycle that pauses. The
// engine's MAX_STEPS_PER_INVOCATION only reschedules (~1–2 s per hop), so two
// executions made 480 tasks + emails before a manual cancel. The engine now
// stops such a run after a few passes (workflow_engine.js runaway-loop
// guard); this is the publish-time half — the static cases never ship.
//
// Graph: one node per step.
//   control step (function listed in branchTargetParams — the map IS the
//     registry of functions whose next_step the engine honors, pinned by
//     tests/control.flow.test.js): edges to its LITERAL targets only —
//     control steps never fall through (control.js evaluate_condition NOTE).
//   any other step: fall-through edge n → n+1.
// Removed before the search:
//   PAUSE_FUNCTIONS nodes — a cycle through a real pause is a legitimate
//     wait-and-retry loop (wf27's SS re-arm path pauses on request_decision).
//   foreach LOOP-BACK edges — a jump onto a foreach from a later step; the
//     cursor bounds that loop (control.js). The foreach node itself stays, so
//     an outer loop that runs through a foreach and jumps back ABOVE it is
//     still a cycle (it would restart the whole list every pass), and so is
//     a body that jumps back to a setup step instead of onto the foreach.
// Any strongly-connected component left (size > 1, or a self-loop) is a loop
// that can repeat with no pause. Removing pause nodes FIRST is load-bearing:
// wf27 v6's runaway cycle sat inside a larger SCC that also contained its
// request_decision step, so an SCC-contains-a-pause test passes it.
//
// Deliberate blind spots (the runtime guard covers them): non-literal targets
// ({{jump_to}}) add no edge; the skip-block path of wait_for / schedule_resume
// (no pause when `at` resolves null) is treated as pausing, and so is a wait
// shorter than the runtime guard's minimum pause; a foreach that THROWS under
// the default 'ignore' policy falls through into its body every visit
// (dropping its loop-back edges assumes the cursor advances).
//
// Policy note: every pause-free loop is rejected, including bounded ones
// (counter / pagination / immediate-retry). The runtime guard would stop any
// such loop after LOOP_GUARD_MAX_BACKJUMPS passes anyway, so the rule is the
// same at both layers: a loop pauses each pass, or it is a foreach.
//
// PAUSE_FUNCTIONS must list exactly the functions that can return
// delayed_until — tests/versionDiff.test.js pins it against the source.
const PAUSE_FUNCTIONS = new Set(['request_decision', 'wait_for', 'schedule_resume', 'wait_until_time']);

function findPauseFreeCycles(draftSteps, branchTargetParams) {
  // 1. The full graph.
  const succ = new Map();
  const removed = new Set();
  const foreachSteps = new Set();
  for (const s of draftSteps) {
    const n = s.step_number;
    const cfg = parseMaybeJson(s.config);
    const fn = s.type === 'internal_function' && cfg && typeof cfg === 'object' ? cfg.function_name : null;
    let targets;
    if (fn && branchTargetParams[fn]) {
      let entries = extractBranchTargets(cfg, branchTargetParams);
      // Branch mode never takes a leftover top-level `then` (control.js
      // evaluate_condition returns from the branches loop / else) — no edge.
      if (fn === 'evaluate_condition' && Array.isArray(cfg.params?.branches)) {
        entries = entries.filter((e) => e.path !== 'evaluate_condition.then');
      }
      targets = entries.map((t) => asLiteralStep(t.value)).filter((t) => t !== null);
      if (fn === 'foreach') targets.push(n + 1); // body entry — foreach jumps into its body explicitly
    } else {
      targets = [n + 1];
    }
    succ.set(n, targets);
    if (fn && PAUSE_FUNCTIONS.has(fn)) removed.add(n);
    if (fn === 'foreach') foreachSteps.add(n);
  }
  // 2. Remove pausing nodes (and every edge into them) and foreach loop-back
  //    edges (a jump onto a foreach from a later step).
  for (const n of removed) succ.delete(n);
  for (const [n, ts] of succ) {
    succ.set(n, [...new Set(ts.filter((t) => succ.has(t) && !(foreachSteps.has(t) && t <= n)))]);
  }

  // Tarjan's SCC. Step counts are small (tens), so recursion depth is fine.
  let index = 0;
  ...
  const strong = (v) => {
    ...
    if (low.get(v) === idx.get(v)) {
      ...
      if (comp.length > 1 || succ.get(v).includes(v)) {
        comp.sort((a, b) => a - b);
        const members = new Set(comp);
        const backJumps = [];
        for (const a of comp) for (const b of succ.get(a)) if (members.has(b) && b <= a) backJumps.push(`${a}→${b}`);
        cycles.push({ steps: comp, backJumps });
      }
    }
  };
  for (const v of [...succ.keys()].sort((a, b) => a - b)) if (!idx.has(v)) strong(v);
  return cycles.sort((a, b) => a.steps[0] - b.steps[0]);
}
...
function validateWorkflowDraft(draftSteps, opts) {
  ...
  for (const c of findPauseFreeCycles(draftSteps, branchTargetParams)) {
    errors.push(
      `steps ${c.steps.join(', ')} form a loop with no pause and no foreach ` +
      `(jump back: ${c.backJumps.join(', ')}) — once entered it repeats every ~1–2 s ` +
      `and re-runs any side effects (tasks, emails, SMS) on each pass. Put a ` +
      `wait_for (at least a minute) / request_decision inside the loop, make it a foreach ` +
      `(the loop-back must land ON the foreach step), or remove the jump back`
    );
  }
