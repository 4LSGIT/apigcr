...
async function deliverWorkflow(target, targetConfig, targetOutput, db) {
  ...
  try {
    ...
    // Refusals — both mean the payload will never reach a step:
    //   inactive          — a hook target pointing at a turned-off workflow is
    //                       a misconfiguration.
    //   never published   — (S3, review D4a) current_version = 0 has nothing to
    //                       run. Pre-versioning this silently created a
    //                       zero-step execution and marked it 'completed'.
    // Both still refuse, but capture runs FIRST (capture-before-publish): this
    // is the exact path an author uses to learn a new workflow's init_data —
    // point a hook at the unpublished workflow, arm capture, fire one real
    // event. The refusal used to run first and the sample was lost, which made
    // the workflow unauthorable and therefore unpublishable.
    const refusalError = wfRow && !wfRow.active
      ? `workflow #${workflowId} is inactive`
      : (wfRow && !wfRow.current_version)
        ? `workflow #${workflowId} has never been published`
        : null;

    if (refusalError) {
      const captured = await captureRefusedStart(db, workflowId, initData);
      if (captured) {
        // The delivery did its job — the payload is now the workflow's sample.
        // Logged 'captured' (not 'failed') so it reads as the authoring step it
        // is and, via `status: 'success'`, does not queue a pointless retry
        // ladder against a workflow that cannot run until someone publishes it.
        logData.response_status = 202;
        logData.response_body = `captured as workflow #${workflowId}'s init_data sample; not started (${refusalError})`;
        logData.status = 'success';
        logData.log_status = 'captured';
        return logData;
      }
      logData.response_status = 409;
      logData.response_body = null;
      logData.status = 'failed';
      logData.error = refusalError;
      return logData;
    }

    // Capture slice — one-shot init_data capture when armed (guarded UPDATE
    // inside the helper; race-free across the four creation sites). 'intercept'
    // is left armed for the step-0 hold in advanceWorkflow.
