 ...
 *   {app_url}/process-job/{id} shortly after the row becomes due
 *   (DISPATCH_BUFFER_MS past it, plus task delivery). The task
 ...
 *   ~105s pre-slice. Post-slice: ~3s per hop. MEASURED start latency
 *   (2026-08-24, 17h post-flip, n=43): ~4–6s typical, matching the
 *   pre-regression 3.8s baseline — with a ~3% tail that silently falls
 *   back to the 60s cron when the awaited createTask RPC starves in the
 *   throttled post-response tail at idle hours (min-instances=0, nothing
 *   else waking the instance). That tail is the standing argument for the
 *   split-phase hook dispatch slice; it is structural, not a bug here.
 *   At ~50 jobs/day this sits deep inside the Cloud Tasks
...
/**
 * Safety buffer added to the task's scheduleTime. Guards one thing: the
 * claim predicate is `scheduled_time <= NOW()` against MySQL's clock, and a
 * task firing a hair early finds 0 rows, burns itself as already_claimed,
 * and the job silently waits for the cron (≤60s) — the exact tail shape the
 * start-latency alert watches.
 *
 * Sized 750ms (was 2000; reduced 2026-08-24 after the post-deploy report
 * showed the buffer was the single largest designed component of the ~4s
 * floor). Basis, both halves verified rather than assumed:
 *   (a) SKEW — measured mysql-minus-GCP at +0.223s with MySQL AHEAD (the
 *       safe direction); 750ms covers a full reversal at ~3x the measured
 *       magnitude plus NOW()'s whole-second truncation.
 *   (b) COMMIT VISIBILITY — every path reaching this module was enumerated
 *       (hookService dispatch, actionDispatchers, composition,
 *       sequenceEngine enroll/advance, workflow_engine delay/self-continue,
 *       process_jobs recovery, and the three route doorbells): ALL use the
 *       autocommit pool, except workflow_engine.parkWithResume (delay /
 *       self-continue, 2026-09-22), which inserts inside a transaction and
 *       rings this doorbell only AFTER commit. The row is committed before
 *       this function is awaited.
 * INVARIANT: if a future caller ever invokes scheduleResume/scheduleStepJob
 * on a TRANSACTION connection, either commit first or raise this buffer —
 * an uncommitted row makes the task a guaranteed already_claimed no-op.
 * Rollback signal if 750ms proves tight: the silent-fallback rate in the
 * alert/verification queries rises; revert this one constant.
 */
const DISPATCH_BUFFER_MS = 750;
...
async function enqueueJobDispatch(jobId, scheduleTime) {
  ...
  return _enqueue({
    ...
    taskName: `d-${jobId}-${Math.floor(dueMs)}`,
    path:     `/process-job/${jobId}`,
    dueMs,
    label:    `job ${jobId}`,
  });
}
...
/**
 * Enqueue a push dispatch for domain_event_queue row `queueId`
 * (POST /process-domain-event/{id}). Fire-and-forget: NEVER rejects.
 *
 * Sibling of enqueueJobDispatch, same doorbell contract, same fallback: if
 * this returns false the row is drained by the batch pass inside the 60s
 * /process-jobs cron instead. lib/domainEvents.emit() must never break
 * because Cloud Tasks is having a bad day.
 *
 * ALWAYS immediate — there is no far-future case here, so no ACCEL_WINDOW_MS
 * gate at the call site. A domain event is by definition due the instant it
 * is emitted; the window exists for scheduled_jobs because that table is a
 * CALENDAR (task reminders 17 days out, recurring rows rescheduled in place)
 * and this one is a work queue.
 *
 * Bare `de-{queueId}` name, no due-time suffix, and that is safe here for a
 * reason that does NOT hold for scheduled_jobs: nothing ever reschedules a
 * queue row in place. Ids are never reused, so the ~1h completed-name
 * retention can never swallow a legitimate second doorbell.
 *
 * @param {number} queueId  domain_event_queue.id (insertId of the fresh row)
 * @returns {Promise<boolean>}  true if a task was created — logging/tests
 *                              only; callers must not branch on it.
 */
async function enqueueDomainEventDispatch(queueId) {
  return _enqueue({
    taskName: `de-${queueId}`,
    path:     `/process-domain-event/${queueId}`,
    dueMs:    Date.now(),
    label:    `domain event ${queueId}`,
  });
}

/**
 * Shared enqueue body. ONE code path for both doorbells — a copy-pasted
 * second one would diverge on the config gate, the never-rejects contract,
 * the projectId cache semantics, or the ALREADY_EXISTS handling, and the
 * divergence would only surface during an outage.
 */
async function _enqueue({ taskName, path, dueMs, label }) {
  try {
    ...
    if (!conf.enabled) {
      if (!warnedOff) {
        warnedOff = true;
        console.log('[taskQueue] cloud_tasks_enabled != 1 — accelerator off, work rides the 60s cron');
      }
      return false;
    }
    ...
    if (!projectIdPromise) {
      ...
      // below → warn → work rides the cron).
    ...
    const url = String(conf.targetBase).replace(/\/+$/, '') + path;
    const parent = client.queuePath(projectId, conf.location, conf.queue);
    const task = {
      name: client.taskPath(projectId, conf.location, conf.queue, taskName),
    ...
    // CPU so this per-item call is a small request on a warm channel.
    await client.createTask({ parent, task }, { timeout: 15000 });
    console.log(`[taskQueue] enqueued dispatch for ${label} (eta ${new Date(etaMs).toISOString()})`);
    return true;
  } catch (err) {
    ...
    console.warn(`[taskQueue] enqueue failed for ${label} (falls back to the cron): ${err.message}`);
    return false;
  }
}
...
module.exports = {
  enqueueJobDispatch, enqueueDomainEventDispatch,
  warmup, ACCEL_WINDOW_MS, DISPATCH_BUFFER_MS, _test,
};
