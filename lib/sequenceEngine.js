...
async function _enrollWithTemplate(db, contactId, template, triggerData) {
  ...
  const [result] = await db.query(
    `INSERT INTO sequence_enrollments
     (template_id, contact_id, appt_id, signing_request_id, trigger_data, status, current_step, total_steps)
     VALUES (?, ?, ?, ?, ?, 'active', 1, ?)`,
    [template.id, contactId, apptIdCol, signingRequestIdForDupCheck, JSON.stringify(triggerData), steps.length]
  );
