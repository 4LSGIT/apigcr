...
async function previewEnrollmentSteps(db, opts = {}) {
  ...
  const [[template]] = await db.query(
    `SELECT id, name, type, current_version, \`condition\` AS template_condition FROM sequence_templates WHERE id = ?`,
    [templateId]
  );
  ...
  // Versioning: enrollment mode previews the enrollment's PINNED version
  // (what will actually fire); template mode previews the published
  // current_version. (S4: template mode becomes draft-else-current.)
  const previewVersion = pinnedVersion ?? template.current_version;
  const [steps] = await db.query(
