...
async function executeStartWorkflowAction(db, enrollmentId, step, enrollment, resolvedConfig) {
  const { resolveExecutionContactId, scheduleResume, captureWorkflowInput } = require('./workflow_engine');
  ...
  if (!wfRow.current_version) {
    throw new Error(`start_workflow step: workflow #${workflowIdNum} has never been published — publish it before targeting it`);
  }
  ...
  // Capture slice — one-shot init_data capture when armed.
