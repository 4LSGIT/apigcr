...
async function executeStep(db, enrollmentId, stepId, stepNumber = null) {
  ...
  if (enrollment._tv_version == null) {
    await logStep(db, enrollmentId, stepId, null, 'failed', `template_version_row_missing (template ${enrollment.template_id} v${enrollment.template_version})`, null, null, null, Date.now() - startTime);
  ...
  if (!steps.length) {
    ...
    await logStep(db, enrollmentId, stepId, stepNum || null, 'failed',
