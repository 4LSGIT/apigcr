...
async function calculateStepTime(timing, triggerData, fromDate, floorDate) {
  ...
  if (timing.type === 'next_business_day') {
    return calendar.nextBusinessDay(from, {
      ...
      minDaysAhead:     timing.minDaysAhead || 0,
    });
  }
...
async function enrollContact(db, contactId, templateType, triggerData = {}) {
  ...
  // Versioning (S4): current_version > 0 keeps never-published templates out
  // of cascade contention entirely — a published sibling can still win.
  // (Explicit ID-based enrollment on a v0 template throws loudly in the
  // funnel instead; there the caller named the template and deserves the
  // error, not a silent skip.)
  const [templates] = await db.query(
    `SELECT * FROM sequence_templates WHERE type = ? AND active = 1 AND current_version > 0`,
    [templateType]
  );
  if (!templates.length) {
...
async function scheduleStepJob(db, enrollmentId, step, scheduledAt) {
  ...
  // Cloud Tasks accelerator (P1, 2026-08) — mirrors scheduleResume in
  // lib/workflow_engine.js. Most sequence steps carry drip delays and stay
  // cron-owned; a zero-delay step (typically an enrollment's first) gets a
  // push dispatch instead of waiting up to 60s for the tick. Awaited —
  // never rejects (test-enforced), bounded ≤15s by the RPC deadline; see
  // the fuller rationale at scheduleResume in lib/workflow_engine.js. Any
  // failure degrades to today's cron latency.
  ...
  if (Number.isFinite(dueMs) && dueMs <= Date.now() + ACCEL_WINDOW_MS) {
    await enqueueJobDispatch(insertResult.insertId, dueMs);
  }
}
...
async function executeStartWorkflowAction(db, enrollmentId, step, enrollment, resolvedConfig) {
  const { resolveExecutionContactId, scheduleResume, captureWorkflowInput, captureRefusedStart } = require('./workflow_engine');
  ...
  // Refusals, thrown below once initData is resolved so an armed capture can
  // still bank the payload (capture-before-publish): a payload that will never
  // reach a step is exactly the sample the target workflow needs to become
  // publishable.
  //
  ...
  const refusal = !wfRow.active
    ? `start_workflow step: workflow #${workflowIdNum} is inactive`
    : !wfRow.current_version
      // Versioning (S3, review D4a): refuse never-published targets loudly —
      // pre-versioning this silently completed a zero-step child execution.
      ? `start_workflow step: workflow #${workflowIdNum} has never been published — publish it before targeting it`
      : null;
  ...
  // Capture slice — one-shot init_data capture when armed. 'intercept' is left
  // armed for the step-0 hold in advanceWorkflow.
  ...
  // Queued start (background-CPU slice, 2026-08) — mirrors
  // actionDispatchers.deliverWorkflow. Previously a detached advanceWorkflow
  // that ran CPU-throttled after the /process-jobs response; now the child
  // execution is handed to the job queue and the next heartbeat (≤60s)
  // advances it request-bound at full CPU. The scheduled job doubles as the
  // crash-durability record. A scheduling failure here throws into this
  // sequence step's own error handling (the step fails loudly instead of
  // silently stranding the child at 'active').
 ...
 */
async function executeStep(db, enrollmentId, stepId, stepNumber = null) {
  const startTime = Date.now();

  // ── Load enrollment ──
  const [enrollments] = await db.query(
    `SELECT e.*, tv.template_condition AS template_condition, tv.version AS _tv_version,
            t.name AS template_name, t.type AS template_type
     FROM sequence_enrollments e
     JOIN sequence_templates t ON t.id = e.template_id
     LEFT JOIN sequence_template_versions tv
            ON tv.template_id = e.template_id AND tv.version = e.template_version
     WHERE e.id = ?`,
    [enrollmentId]
  );
  ...
  // Versioning (S4): the template condition now comes from the enrollment's
  // PINNED version row — a condition edited on the draft cannot retro-gate
  // runs pinned to the published version. LEFT JOIN + explicit check rather
  // than INNER: a missing metadata row is an invariant breach (migration
  // backfilled v1; ensureDraft/publish maintain it), and it should fail with
  // an accurate reason — not masquerade as 'enrollment_not_found', and not
  // silently run ungated.
  if (enrollment._tv_version == null) {
    // Diagnostics contract (reviews F8 + II.1 + II.2):
    //   step_number — the REAL step number from the job payload when we have
    //     it, 0 only as the "genuinely unknown" sentinel. /recover derives
    //     its resume target as last-logged step_number + 1, so a literal 0
    //     here made recovery RESTART FROM STEP 1 — re-sending already-sent
    //     client messages. (step_number is NOT NULL with no default, and a
    //     single-row explicit-NULL insert errors under ANY sql_mode, which
    //     logStep's catch would swallow — hence non-null always.)
    //   message — in the errorMessage slot (last arg), matching the house
    //     failure pattern; the alert scanner reads l.error_message for the
    //     alert body, and skip_reason is for SKIPS.
    await logStep(db, enrollmentId, stepId, Number(stepNumber) || 0, 'failed', null, null, null, null, Date.now() - startTime,
      `template_version_row_missing (template ${enrollment.template_id} v${enrollment.template_version})`);
    return { status: 'failed', reason: 'template_version_row_missing' };
  }
  ...
  if (enrollment.status !== 'active') {
    // 0, not null: a single-row explicit-NULL insert into the NOT NULL
    // step_number column errors under ANY sql_mode (review II.4 — this audit
    // row had never actually been written), and logStep swallows the error.
    await logStep(db, enrollmentId, stepId, 0, 'skipped', 'enrollment_not_active', null, null, null, Date.now() - startTime);
    return { status: 'skipped', reason: 'enrollment_not_active' };
  }

  // ── Load step ──
  // Versioning (S4.1): resolve by (template, PINNED version, step_number) —
  // step_number is a queued step's IDENTITY (scheduleStepJob's idempotency
  // key already says so), and published versions are immutable, so the pair
  // is stable for the enrollment's whole life. This is what lets a
  // content-only publish repoint enrollments WITHOUT rewriting queued jobs:
  // the number resolves to the NEW version's row at fire time, closing the
  // claimed-'running' race the old JSON_SET stepId remap could never cover
  // (the remap only saw status='pending'; a job claimed mid-publish executed
  // with a dead stepId and wedged the enrollment silently).
  //
  // stepId remains the fallback for legacy payloads without stepNumber,
  // still scoped to the pinned (template, version) so a stale pointer
  // degrades to the LOGGED failure below — never another version's step.
  const stepNum = Number(stepNumber);
  let steps;
  if (Number.isInteger(stepNum) && stepNum > 0) {
    [steps] = await db.query(
      `SELECT * FROM sequence_steps WHERE template_id = ? AND version = ? AND step_number = ?`,
      [enrollment.template_id, enrollment.template_version, stepNum]
    );
  } else {
    [steps] = await db.query(
      `SELECT * FROM sequence_steps WHERE id = ? AND template_id = ? AND version = ?`,
      [stepId, enrollment.template_id, enrollment.template_version]
    );
  }

  if (!steps.length) {
    // Log it — a wedged enrollment whose only trace is a console line is not
    // diagnosable. This row is what makes the recover route findable.
    // Same diagnostics contract as template_version_row_missing above:
    // non-null step_number (stepNum || 0), message in the errorMessage slot.
    await logStep(db, enrollmentId, stepId, stepNum || 0, 'failed', null, null, null, null, Date.now() - startTime,
      `step_not_found (template ${enrollment.template_id} v${enrollment.template_version}, step_number ${stepNum || 'n/a'}, stepId ${stepId})`);
...
async function scheduleFromStep(db, enrollment, startStepNumber, triggerData, floorDate) {
  // Versioning (2026-08): the walk reads the enrollment's PINNED template
  // version. `enrollment` here is sometimes a real sequence_enrollments row
  // (executeStep's e.* join) and sometimes a hand-built context object
  // (_enrollWithTemplate's enrollmentCtx) — audit class CONTEXT-CONSTRUCTION.
  // Fail-loud on purpose (review D6): a context object missing
  // template_version would otherwise bind undefined, match zero rows, and
  // silently complete the enrollment at step 1. Guard chosen over a
  // signature change: identical runtime loudness, five fewer churned call
  // sites (deliberate divergence from the review's prescription).
  const version = enrollment.template_version;
  if (!Number.isInteger(version) || version < 1) {
    throw new Error(`scheduleFromStep: enrollment.template_version must be a positive integer (got ${JSON.stringify(version)}) — every enrollment/context object must carry the pinned version`);
  }
  ...
  for (let guard = 0; guard < cap; guard++) {
    const [rows] = await db.query(
      `SELECT * FROM sequence_steps WHERE template_id = ? AND version = ? AND step_number = ?`,
      [enrollment.template_id, version, stepNumber]
    );
...
async function _enrollWithTemplate(db, contactId, template, triggerData) {
  // Capture slice — one-shot capture of trigger_data, mirroring the
  // workflows/hooks pattern. `template` rows arrive via SELECT * from both
  // callers, so capture_mode is always present. Captured at ingress (before
  // the duplicate-enrollment guard): a dup-rejected enrollment still shows
  // the author what real trigger_data looks like, which is the point.
  // Guarded UPDATE = race-free one-shot; failures never break enrollment.
  if (template.capture_mode === 'capturing') {
    try {
      await db.query(
        ...
        [JSON.stringify(triggerData ?? {}), template.id]
      );
    } catch (err) {
      console.warn(`[SEQ CAPTURE] capture failed for template ${template.id}:`, err.message);
    }
  }

  // ── Load steps ──
  // Versioning: read the version ONCE and bind it as a literal to BOTH the
  // step load and the enrollment INSERT below (review D3). This function runs
  // on the pool with no transaction, so a publish landing between the two
  // statements must not be able to split them across versions: total_steps,
  // the stamped template_version, and the first scheduled stepId all derive
  // from this one value. (The benign residue: enrolling on the version that
  // was current a moment before a publish — coherent, since published
  // versions are immutable.)
  //
  // FAIL-LOUD (S4): this is the single funnel both enroll paths pass through,
  // so it is also the never-published refusal (review D4a for sequences). The
  // old `|| 1` fallback would have silently enrolled a v0 template onto v1 —
  // which post-S4 is the DRAFT.
  const templateVersion = Number(template.current_version);
  if (!Number.isInteger(templateVersion) || templateVersion < 1) {
    throw new Error(`enroll: sequence template #${template.id} has never been published — publish it before enrolling`);
  }
  const [steps] = await db.query(
    `SELECT * FROM sequence_steps WHERE template_id = ? AND version = ?
     ORDER BY step_number ASC`,
    [template.id, templateVersion]
  );
  ...
  // Phase 3 extends the same rule to esign reminder sequences via
  // trigger_data.signing_request_id: one client with two live signing
  // requests holds two independent enrollments. For contact-scoped sequences
  // (neither id present), "duplicate" stays contact-level. MySQL's <=>
  // operator handles all cases: NULL <=> NULL = 1, any non-equal pair = 0.
  const apptIdForDupCheck = triggerData?.appt_id ? Number(triggerData.appt_id) : null;
  const signingRequestIdForDupCheck =
    triggerData?.signing_request_id ? Number(triggerData.signing_request_id) : null;
  const [existing] = await db.query(
    `SELECT id FROM sequence_enrollments
     WHERE contact_id = ?
       AND template_id = ?
       ...
       AND status = 'active'
     LIMIT 1`,
    [contactId, template.id, apptIdForDupCheck, signingRequestIdForDupCheck]
  );
  if (existing.length) {
    const ctx = apptIdForDupCheck ? ` for appt ${apptIdForDupCheck}`
      : signingRequestIdForDupCheck ? ` for signing request ${signingRequestIdForDupCheck}` : '';
  ...
  // lookup instead of JSON_EXTRACT. signing_request_id (Phase 3 migration)
  // follows the identical pattern for esign reminder enrollments. Coerce
  // 0/'' to NULL via the truthiness checks — those are not meaningful IDs.
  const apptIdCol = triggerData?.appt_id ? Number(triggerData.appt_id) : null;

  const [result] = await db.query(
    `INSERT INTO sequence_enrollments
     (template_id, template_version, contact_id, appt_id, signing_request_id, trigger_data, status, current_step, total_steps)
     VALUES (?, ?, ?, ?, ?, ?, 'active', 1, ?)`,
    [template.id, templateVersion, contactId, apptIdCol, signingRequestIdForDupCheck, JSON.stringify(triggerData), steps.length]
  );
  ...
  const enrollmentCtx = {
    ...
    template_version: templateVersion,   // CONTEXT-CONSTRUCTION: scheduleFromStep throws without it
...
async function previewEnrollmentSteps(db, opts = {}) {
  ...
  let pinnedVersion = null; // set in enrollment mode: preview what WILL run
  if (opts.enrollment_id != null) {
    const [[enr]] = await db.query(
      `SELECT template_id, template_version, contact_id, trigger_data FROM sequence_enrollments WHERE id = ?`,
      [opts.enrollment_id]
    );
    if (!enr) throw new Error(`Enrollment #${opts.enrollment_id} not found`);
    templateId  = enr.template_id;
    pinnedVersion = enr.template_version;
  ...
  // ── Load template (name, type) + resolve preview version ──
  const [[template]] = await db.query(
    `SELECT id, name, type, current_version, draft_version FROM sequence_templates WHERE id = ?`,
    [templateId]
  );
  ...
  // Versioning (S4): enrollment mode previews the enrollment's PINNED version
  // (what will actually fire); template mode previews the DRAFT when one
  // exists, else the published version — authors preview what they're
  // editing. The template condition comes from the SAME version row, so the
  // gate previewed matches the steps previewed.
  const previewVersion = pinnedVersion ?? (template.draft_version ?? template.current_version);
  ...
  const templateCondition = condRow && typeof condRow.template_condition === 'string'
    ? JSON.parse(condRow.template_condition)
    : (condRow ? condRow.template_condition : null);
  const [steps] = await db.query(
