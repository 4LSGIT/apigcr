...
async function executeStep(db, enrollmentId, stepId) {
  ...
  // ── Load step ──
  // Versioning (S4 hardening): the scheduled job's stepId is trusted data,
  // and the migration path now REWRITES stepIds — constrain the load to the
  // enrollment's pinned (template_id, version) so a bad remap degrades to a
  // logged skip instead of firing another version's step.
