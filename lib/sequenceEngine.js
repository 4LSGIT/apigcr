...
async function enrollContact(db, contactId, templateType, triggerData = {}, filters = {}) {
  ...
  // The ORDER BY puts the most specific match first:
  //   both filters matched > one filter matched > generic (both NULL)
...
async function executeStep(db, enrollmentId, stepId) {
  ...
  while (true) {
    try {
      // executeJob requires a 'type' field. All sequence actions use
      // internal_function. Webhooks are also supported if action_config has a url.
      const jobData = {
        type: step.action_type === 'webhook' ? 'webhook'
            : step.action_type === 'custom_code' ? 'custom_code'
            : 'internal_function',
        ...resolvedConfig
      };
