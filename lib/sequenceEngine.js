...
// Grace window for before_appt_fixed: a step whose computed fire time is at
// most this far in the past (relative to the scheduling moment) still fires,
// rather than being skipped. Absorbs the ~1-min job-poll lag so a 3-min-before
// step computed a few seconds late isn't dropped. Steps further past than this
// are treated as "window gone" → skipped.
...
async function calculateStepTime(timing, triggerData, fromDate) {
  ...
  if (timing.type === 'before_appt_fixed') {
    ...
    // If the computed time has already passed at the moment we're scheduling
    // (`from`), this step's window is gone — return null to signal "skip".
    // The scheduler walks forward to the next still-placeable step. Lets a
    // template carry steps (e.g. 7-day / 2-day / 24h / 2h) and silently drop
    // the ones whose runway is gone for short-notice bookings. A small slop is
    // allowed so a step a few seconds past due (1-min job poll) still fires.
    if (scheduled.getTime() < from.getTime() - SCHEDULE_PAST_SLOP_MS) return null;
    return scheduled;
  }
...
//     "Recover stuck enrollment" buttons in contact2.html and the
//     sequences manager)
...
// ─────────────────────────────────────────────────────────────
async function safeAdvanceToNextStep(db, enrollment, completedStep, triggerData, startTime) {
  try {
    return await advanceToNextStep(db, enrollment, completedStep, triggerData, startTime);
  } catch (err) {
    console.error(
...
async function executeStep(db, enrollmentId, stepId) {
  ...
  if (!checkFireGuard(step.fire_guard, triggerData)) {
    ...
    await safeAdvanceToNextStep(db, enrollment, step, triggerData, startTime);
  ...
  const nextScheduledAt = await safeAdvanceToNextStep(db, enrollment, step, triggerData, startTime);
...
// ─────────────────────────────────────────────────────────────
async function scheduleFromStep(db, enrollment, startStepNumber, triggerData) {
  ...
  for (let guard = 0; guard < cap; guard++) {
    ...
    const scheduledAt = await calculateStepTime(step.timing, triggerData, new Date());
...
async function advanceToNextStep(db, enrollment, completedStep, triggerData) {
  return scheduleFromStep(db, enrollment, completedStep.step_number + 1, triggerData);
}
...
async function _enrollWithTemplate(db, contactId, template, triggerData) {
  ...
  const scheduledAt = await scheduleFromStep(db, enrollmentCtx, 1, triggerData);
