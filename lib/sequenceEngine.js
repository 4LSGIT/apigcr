...
async function executeStep(db, enrollmentId, stepId, stepNumber = null) {
  ...
  if (enrollment._tv_version == null) {
    // step_number 0 = explicit sentinel: sequence_step_log.step_number is NOT
    // NULL with no default, and logStep's catch swallows insert errors — under
    // strict sql_mode (Cloud SQL migration) a raw null would silently drop
    // this diagnostic row, which is the one trace of the wedge (review F8).
    await logStep(db, enrollmentId, stepId, 0, 'failed', `template_version_row_missing (template ${enrollment.template_id} v${enrollment.template_version})`, null, null, null, Date.now() - startTime);
  ...
  if (!steps.length) {
    ...
    // stepNum || 0 — same strict-mode sentinel as above (review F8).
    await logStep(db, enrollmentId, stepId, stepNum || 0, 'failed',
