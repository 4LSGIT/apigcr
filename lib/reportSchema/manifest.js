...
//
// FILL-RATE FIGURES were measured against live data on 2026-07-28 and will
// drift. They are indicative, not contractual — they exist to warn, not to
// compute with.
...
const GLOBAL_NOTES = [
  ...
  "ZERO-DATE HAZARD: several date columns still contain legacy '0000-00-00' values, but NO_ZERO_DATE means comparing a DATE column to the literal '0000-00-00' throws ER_WRONG_VALUE. The filter that WORKS is: CAST(col AS CHAR) <> '0000-00-00'. Writing `WHERE col <> '0000-00-00'` is a hard error, and `WHERE col IS NOT NULL` does NOT exclude them.",

  "There is NO stage-history or audit table for cases. Anything phrased as time-in-stage, stage transitions, funnel conversion between stages, or 'as of date X' is UNANSWERABLE. Say so rather than approximating it from the current stage. The only usable progression signals are the three date columns case_open_date / case_file_date / case_close_date, and case_file_date is populated on only ~16% of cases.",
...
const TABLES = {
  ...
  cases: {
    ...
    columns: {
      ...
      case_stage: {
        ...
        note:
          "CURRENT stage only — there is no history. Live distribution is " +
          "heavily Open (~707) and Closed (~302), with Filed ~45 and Pending " +
          "~14. Note 'Concluded' is a legal enum value with zero rows.",
      },
      ...
      case_file_date: { type: "date", note: "Petition filing date. Populated on only ~16% of cases (169 real values), plus 5 rows holding a 0000-00-00 zero-date. Use the CAST AS CHAR filter from the global notes. One case has a file date EARLIER than its open date, so a naive MIN(DATEDIFF(...)) over this pair returns a negative number — mention that in caveats or filter it out explicitly." },
      case_close_date: { type: "date", note: "Populated on ~27% of cases." },
      case_source: {
        type: "varchar(20)",
        note:
          "NOT a marketing lead source, despite the name. It records the " +
          "intake channel. Blank on ~41% of rows; the dominant value is the " +
          "literal string 'https://api.calendly' (~566 rows) meaning 'booked " +
          "through Calendly'. Do NOT build lead-source or marketing-attribution " +
          "reports on this column — say the data isn't captured yet.",
      },
      ...
      "341_status": { type: "enum('Continued','Completed')", note: "Column name STARTS WITH A DIGIT — must be backtick-quoted in SQL: `341_status`. Same for `341_appt_id`, `341_docs`, `341_amend`, `341_notes`." },
  ...
  events: {
    ...
    columns: {
      ...
      event_status: { type: "enum('Scheduled','Completed','Canceled')" },
