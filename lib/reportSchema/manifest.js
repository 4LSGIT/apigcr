...
// (credential material).
//
// contact_ssn and contact_dob were on this list until 2026-09-24 and are not
// any more (Fred's ruling). A bankruptcy firm files Form 121; staff read the
// SSN all day and a report that cannot show a date of birth cannot answer
// ordinary questions. What remains here is credential material and tokens
// that grant access if leaked — things no report needs at any time, rather
// than things that merely feel sensitive.
// ─────────────────────────────────────────────────────────────────────────────
...
const DENIED_COLUMNS = [
  ...
  "task_action_token",
  "draft_key",
  // Not a secret — the raw custom-fields JSON bag on cases AND contacts
  // (custom-fields S2, ref/CUSTOM_FIELDS_DESIGN.md §2–§3). A JSON-path read
  // of it (->>) compares case-sensitively and can flip results when an index
  // is added;
  // S3 gives every field a named, collated virtual column to report on
  // instead. Denied as an identifier, so every JSON path into it is too.
  "custom",
];
...
const TABLES = {
  ...
  cases: {
    note:
      ...
      "dimension. PK is case_id, a VARCHAR — not an integer. The table also " +
      "has a JSON column `custom` (admin-defined custom fields) that is on the " +
      "column denylist and cannot appear in a report, not even via a JSON path. " +
      "The individual custom fields ARE reportable, each as its own named " +
      "`cf_*` column — they are listed, with their types and option values, in " +
      "the generated 'Admin-defined custom fields' section appended below " +
      "(lib/reportSchema/customFieldsAppendix.js). Nothing is listed there when " +
      "the firm has defined none.",
    columns: {
      ...
      case_judge: { type: "varchar(100)", note: "Judge NAME as free text (authoritative display value). Canonical link is case_judge_contact_id → contacts (contact_roles role='judge'); the old judges table is dropped." },
      case_trustee: { type: "varchar(100)", note: "Trustee NAME as free text (authoritative display value). Canonical link is case_trustee_contact_id → contacts (contact_roles role='trustee'); the old trustees table is dropped. Populated on ~16% of rows." },
      ...
      clio_matter: { type: "varchar(20)", note: "RETIRED 2026-09-25 (custom fields S5) and FROZEN — no longer written. Its 240 values were migrated to the custom field cf_clio_matter, which is the live Clio matter number and appears in the custom-fields appendix. Report on cf_clio_matter, never on this column; it is dropped in S5-B." },
  ...
  contacts: {
    note:
      "People AND organizations — clients, leads, and other parties. ~1,009 rows. " +
      "contact_kind ('person'|'org') separates the two; org rows carry a blank " +
      "contact_fname/contact_lname and name themselves via contact_org_name. " +
      "contact_ssn and contact_dob are selectable (2026-09-24 ruling) — see their column notes for the format traps. " +
      "The JSON column `custom` (admin-defined custom fields) is on the column denylist and cannot appear in a " +
      "report, not even via a JSON path. The individual custom fields ARE reportable, each as its own named " +
      "`cf_*` column — see the generated 'Admin-defined custom fields' section appended below.",
    columns: {
      contact_id: { type: "int unsigned", note: "PK." },
      contact_kind: { type: "varchar(12)", note: "THE ENTITY AXIS: 'person' or 'org'. This — not contact_type — is what tells you whether a row is a human or a company. Org rows have BLANK contact_fname / contact_mname / contact_lname; their name lives in contact_org_name and is mirrored verbatim into all three derived name columns. Filter on this before grouping or splitting on name parts." },
      contact_org_name: { type: "varchar(120)", note: "Organization name. Non-empty only when contact_kind = 'org'; '' on person rows. Mirrored verbatim (no comma flip) into contact_name / contact_lfm_name / contact_rname." },
      contact_type: { type: "varchar(20)", note: "Free text ROLE-ish label, NOT the person/org axis — use contact_kind for that. Inconsistent; as of 2026-09-08 the distinct values are 'person' (950), 'Client' (74) and '' (26), and the value 'person' here has nothing to do with contact_kind = 'person'. Two routes disagree on the default for new rows (api.intake writes 'Client', booking writes 'person'). Inspect distinct values before grouping on it." },
      contact_name: { type: "varchar(120)", note: "Display name, trigger-computed. Person: 'First M Last'. Org: the org name verbatim. contact_fname / contact_lname hold the parts on person rows only." },
      contact_fname: { type: "varchar(20)", note: "BLANK on org rows." },
      contact_lname: { type: "varchar(30)", note: "BLANK on org rows." },
      ...
      contact_dob: { type: "date", note: "NULLable and mostly NULL — 189 of 1,088 rows have one (2026-09-24). Selectable since the 2026-09-24 ruling. Age arithmetic must exclude NULLs explicitly; COUNT(contact_dob) and COUNT(*) are very different numbers here." },
      contact_ssn: { type: "char(11)", note: "SSN for persons, EIN for orgs — ONE column serving both, by design. Selectable since the 2026-09-24 ruling. NOT NULL but usually '': 151 of 1,088 rows are non-empty (2026-09-24), so filter on contact_ssn <> '', never IS NOT NULL. FORMAT IS INCONSISTENT — staff type it both ways and 14 of the 151 carry dashes ('123-45-6789' vs '123456789'), so equality matching and LENGTH() are both unreliable; strip non-digits before comparing. A report that prints this is printing a real SSN — prefer RIGHT(<digits>, 4) unless the whole number is the point." },
  ...
  // The `trustees` and `judges` reference tables were DROPPED (slice 7B / m6,
  // 2026-09-23). Judge/trustee identity now lives on contacts + contact_roles
  // (roles 'judge' / 'trustee'), linked from cases via the *_contact_id twins.
};
