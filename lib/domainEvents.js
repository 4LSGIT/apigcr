 ...
 *   into trigger_executions.envelope and must not replicate SSNs. So is
 *   `custom` (cases.custom / contacts.custom, custom-fields S2): the JSON bag
 *   never travels whole — per-field `changes` name cf_ keys individually, and
 *   S3's virtual columns bring the values into row snapshots one column each.
 *
...
const REDACT_KEYS = new Set([
  ...
  // Custom-fields S2 (ref/CUSTOM_FIELDS_DESIGN.md §3): the raw JSON bag on
  // cases AND contacts. Name-based and table-agnostic like the rest — no other
  // table has a `custom` column. Not a secret: the bag must not travel whole.
  'custom',
]);
