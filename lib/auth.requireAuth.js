...
//   3. Global kill switch: app_settings.portal_live — '0' means the portal
//      is DOWN (uniform 401 here; the login endpoints give a friendly 503).
//      Absent or anything else = live: the switch postdates the portal, so
//      a missing row must never take it down (fail-open).
//   4. One indexed contacts read: portal_enabled must be 1 and token ver
//      must equal portal_session_version (bump = all-device revoke).
//   5. req.auth = { type: 'portal', contactId }.
//   EVERY failure → uniform 401 { error: 'Unauthorized' } — no branch
//   distinguishability (no oracle for token-vs-disabled-vs-revoked-vs-down).
...
// ── global portal kill switch ───────────────────────────────────────────────
// app_settings.portal_live: '0' = portal down; absent/'1'/anything else =
// live (fail-open — see doc above). Lives HERE (not portalAuthService) to
// avoid a require cycle: portalAuthService already requires this module for
// _insertPortalAccessLog. portal.auth.js imports it for the login gates so
// there is exactly ONE reading of the switch.

const PORTAL_LIVE_KEY = 'portal_live';

async function isPortalLive(db) {
  const v = await getSetting(db, PORTAL_LIVE_KEY);
  return v === null || String(v).trim() !== '0';
}
...
function contactMiddleware(req, res, next) {
  ...
  (async () => {
    ...
    // 3. Global kill switch — after token checks (junk requests never cost
    //    the extra read), before the contacts read. Uniform 401.
    if (!(await isPortalLive(req.db))) return fail();

    // 4. Enabled + session-version check (one indexed PK read).
...
module.exports.isPortalLive = isPortalLive;       // shared with portal.auth.js login gates
module.exports.PORTAL_LIVE_KEY = PORTAL_LIVE_KEY;
