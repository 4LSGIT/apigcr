...
- hearing_adjourned   — a hearing adjourned/rescheduled/continued, or canceled outright.
...
cancel_event — the email states a previously-set hearing/event is DISSOLVED, vacated,
  ...
  let the executor reconcile. A cancellation with NO replacement date is the NORMAL
  cancel_event case, not a review case: "date":null is expected and the executor matches
  on event_type/event_title without it. The absence of a new date is NEVER by itself a
  reason to set needs_review. This is NOT limited to show-cause orders — it applies to any
  canceled/vacated/terminated hearing, conference, or pre-trial (e.g. "Minute Entry. Final
  Pre-trial Conference canceled due to mediation"). Do NOT use cancel_event for a reschedule
  (that is update_event) or for a case dismissal/discharge/closing (those are
...
cannot confidently map; OR a record likely must be CANCELED with no clear replacement AND you
did NOT emit a cancel_event for it (e.g. a stipulation you cannot map to a specific hearing).
HARD RULE — if your "actions" array contains a cancel_event, needs_review MUST be false unless
some OTHER, UNRELATED part of the same email is unclear. Emitting cancel_event and setting
needs_review for the same cancellation is a contradiction: you have already handled it.
"No replacement/new date was given" is NOT a review reason — that is what cancel_event means.
...
EXAMPLE E2 (hearing canceled outright, no new date — NOT review):
Body excerpt: 'Case Name: Schneyer v. Childs  Case Number: 25-04092-prh [...] Docket Text: Minute
Entry. Final Pre-trial Conference canceled due to mediation. (RE: related document(s)14
Scheduling Order) (lbryant)'
Output:
{ "message_id":"{{message_id}}","case_number":"25-04092-prh","case_name":"Schneyer v. Childs",
 "chapter":null,"classification":"hearing_adjourned","needs_review":false,"review_reason":null,
 "actions":[{"type":"cancel_event","fields":{"event_type":"Pre-trial Conference",
   "event_title":"Final Pre-trial Conference","date":null},
  "citations":{"event_type":"Final Pre-trial Conference canceled due to mediation",
   "event_title":"Final Pre-trial Conference canceled due to mediation"}}],
 "notes":"Pre-trial conference canceled with no replacement date. cancel_event handles it; NOT review." }
