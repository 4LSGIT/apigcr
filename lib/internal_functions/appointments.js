...
fns.create_appointment = async (params, db) => {
    ...
    const {
      ...
      // External-system reference + channel. apptService.createAppt has always
      // accepted both; this function silently dropped them, which is why every
      // Calendly appt with appt_ref_id/appt_source was written by Pabbly via
      // POST /api/appts rather than by a workflow step.
      appt_ref_id     = '',
      appt_source     = null,
    } = params;
...
fns.create_appointment.__meta = {
  ...
  params: [
    ...
    { name: 'appt_ref_id', type: 'string', required: false, placeholderAllowed: true,
      description: 'External-system reference (Calendly scheduled_events URL, etc). Stored on appts.appt_ref_id, clamped to 100 chars by apptService. This is the per-appointment cancel handle — prefer it over cases.case_source_ref, which a second booking would clobber.',
      example: '{{leadRefId}}' },
    { name: 'appt_source', type: 'string', required: false, placeholderAllowed: true,
      description: 'Booking CHANNEL, stored on appts.appt_source (varchar 60) — e.g. "calendly". Distinct from `source` above, which is the audit/alert origin enum.',
      example: 'calendly' },
  ],
...
fns.update_appointment = async (params, db) => {
    ...
    // U6b — the anchor pair is DERIVED (v0.5 A3a): a patched appt_case_id /
    // appt_client_id rewrites appt_link_type/appt_link_id; the raw columns
    // themselves stay off the allowlist above. Same helper as the PATCH
    // route, so the two paths cannot diverge.
    const apptSvc = require('../../services/apptService');   // deferred require (circular dep safety)
    const withLinks = await apptSvc.applyApptLinkPatch(db, appointment_id, resolved);
    const keys = Object.keys(withLinks);
 
    console.log(`[UPDATE_APPOINTMENT] id=${appointment_id} fields=${JSON.stringify(withLinks)}`);
 
    const setClauses = keys.map(k => `\`${k}\` = ?`).join(', ');
    const values = [...keys.map(k => withLinks[k]), appointment_id];
...
// ─────────────────────────────────────────────────────────────
// find_live_calendar_item (U6b — read-only)
// ─────────────────────────────────────────────────────────────
/**
 * find_live_calendar_item
 *
 * "Is there already a live calendar row for this identity?" — the singleton
 * question, answered across BOTH tables (events + appts) through
 * calendarWriteService.findLive (v0.5 §3.4.2 semantics: identity is the
 * resolved case + type_key; docket anchors resolve first; a contact anchor
 * matches events by raw identity and never matches appts). Read-only: this
 * function writes nothing.
 */
fns.find_live_calendar_item = async (params, db) => {
    const { type_key, case_id = null, contact_id = null, docket = null } = params;
    if (!type_key) throw new Error('find_live_calendar_item requires type_key');
    const calendarWriteService = require('../../services/calendarWriteService');   // deferred require (circular dep safety)
    const items = await calendarWriteService.findLive(db, {
      type_key,
      anchor: { case_id, contact_id, docket },
    });
    return { success: true, output: { items, count: items.length } };
  };

fns.find_live_calendar_item.__meta = {
  category: 'appointments',
  description: 'Find live (Scheduled) calendar rows — events AND appts — matching a singleton identity: registry type_key + an anchor (case_id, docket, or contact_id). Read-only. Returns { items: [{ source, id, starts_at }], count }.',
  params: [
    { name: 'type_key', type: 'string', required: true,
      description: 'calendar_item_types.type_key, e.g. meeting_341.',
      example: 'meeting_341' },
    { name: 'case_id', type: 'string', required: false, placeholderAllowed: true,
      description: 'Case anchor. Wins over docket/contact when present.',
      example: '{{link_id}}' },
    { name: 'docket', type: 'string', required: false, placeholderAllowed: true,
      description: 'Docket anchor (either case_number form). Resolved to its case when one exists.',
      example: '{{court_case_number}}' },
    { name: 'contact_id', type: 'string', required: false, placeholderAllowed: true,
      description: 'Contact anchor (events only — appts have no contact singleton identity).',
      example: '{{primary_contact_id}}' },
  ],
  example: { type_key: 'meeting_341', docket: '{{court_case_number}}' }
};
