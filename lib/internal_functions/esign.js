...
/**
 * When was a webhook delivery last PROVEN to have arrived?
 *
 * Primary source is the heartbeat setting, stamped by routes/api.esign.js for
 * every delivery that clears both auth gates — including ones the pipeline
 * later noops or cannot match, because the signal is "Zoho reached us and was
 * let in", not "we understood it".
 *
 * FALLBACK: that setting did not exist before the heartbeat shipped, and an
 * absent row read as "no delivery EVER" — which is how task 1077 came to
 * announce a dead channel on 2026-07-27 while three webhook-sourced events
 * landed within nine minutes of it. So when the setting is missing or
 * unparseable we fall back to the newest webhook-sourced audit row: a coarser
 * record of the same fact that covers the whole pre-heartbeat era. Only when
 * BOTH are empty has a delivery genuinely never been seen.
 *
 * The fallback's precision is limited by how signing_request_events.occurred_at
 * is stored and returned (DATETIME, no zone), which is acceptable for a
 * 24-hour staleness test and never used when the heartbeat is available.
 *
 * Never throws — a dead-channel check must not fail a run whose real work is
 * done. An unreadable source is treated as absent, and the caller's remaining
 * source (or the 'never' branch) decides.
 *
 * @returns {Promise<{at: Date|null, via: 'heartbeat'|'audit'|null}>}
 */
async function lastProvenDelivery(db) {
  try {
    const raw = await getSetting(db, esignWebhookService.WEBHOOK_LAST_SEEN_KEY);
    if (raw) {
      const d = new Date(raw);
      if (!Number.isNaN(d.getTime())) return { at: d, via: 'heartbeat' };
    }
  } catch (err) {
    console.warn(`[ESIGN RECONCILE] heartbeat unreadable, falling back to audit: ${err.message}`);
  }
  ...
  try {
    const [[row]] = await db.query(
      `SELECT MAX(occurred_at) AS at
        ...
        WHERE JSON_UNQUOTE(JSON_EXTRACT(payload, '$.source')) = 'webhook'`
    );
    if (row && row.at) {
      const d = new Date(row.at);
      if (!Number.isNaN(d.getTime())) return { at: d, via: 'audit' };
    }
  } catch (err) {
    console.warn(`[ESIGN RECONCILE] audit fallback failed: ${err.message}`);
  }
...
/**
 * Statuses whose arrival is CARRIED BY A WEBHOOK ahead of this job on a
 * healthy channel. Only a move INTO one of these counts as evidence that a
 * webhook was missed.
 *
 * ── WHY 'expired' IS NOT HERE ───────────────────────────────────────────────
 * Zoho DOES emit an expiry webhook — that was verified, not assumed:
 *
 *   request 30, expires_at 2026-08-01T23:20:26Z
 *     2026-08-02T11:00:04Z  reconcile applied  sent → expired
 *     2026-08-02T21:30:25Z  webhook arrived, "request 30: expired — no
 *                           change (noop)"  (Cloud Run, revision …01047-z6k)
 *
 * The webhook was ~22h behind the expiry and ~10h behind this job. Both paths
 * worked; reconcile simply got there first. That is not an accident of timing
 * — it is STRUCTURAL. Zoho's expiry notification is batched hours after the
 * clock runs out, while this job polls every outstanding row nightly, so
 * reconcile wins the race essentially every time. The event history bears it
 * out: 0 of 20 webhook-delivered events have ever been an expiry, and all 4
 * recorded expiries came from reconcile.
 *
 * So a reconcile-applied expiry is the EXPECTED outcome on a perfectly
 * healthy channel, and counting it as a missed webhook made the conjunction
 * fire precisely when nothing was wrong: a quiet shop produces webhook
 * silence AND leaves expiry as the only move reconcile can make. Observed
 * false positives: task 1077 (2026-07-27), task 1114 (2026-08-15).
 *
 * 'recalled' stays in — we initiate it, but Zoho notifies on it promptly.
 * 'satisfied_external' is ours alone and never crosses the wire.
 */
const WEBHOOK_CARRIED_TRANSITIONS = new Set([
  'viewed', 'partially_signed', 'signed', 'declined', 'bounced', 'recalled',
]);
...
fns.esign_reconcile = async (params = {}, db) => {
  ...
  const summary = {
    // moved       = every status change reconcile applied.
    // actor_moved = the subset a webhook should have delivered first (see
    //               WEBHOOK_CARRIED_TRANSITIONS). Only this drives the
    //               dead-channel alert; `moved` stays the honest total.
    checked: 0, moved: 0, actor_moved: 0, filed: 0, unchanged: 0, failed: 0, refiled: 0,
  ...
  try {
    ...
    await esignAlertService.raiseTask(db, {
      // Nightly job, identical title every run: a broken credential produced
      // one task per night for as long as it stayed broken. Keyed so the
      // second and later nights touch the first task instead. Literal rather
      // than esignAlertService.DEDUPE_KEYS.RECONCILE_PROVIDER because this
      // module is exercised against a mock of that service — keep in sync.
  ...
  // The provider BUILT, so whatever broke it is fixed — close the task that
  // said otherwise. This is the only unambiguous clearing signal the job has:
  // a credential that resolves is a credential that works.
  //
  // Deliberately NOT done for 'reconcile-failures' at the end of this function.
  // A per-row failure that does not recur tonight is not necessarily HANDLED —
  // the envelope may simply have expired out of the outstanding set — and
  // auto-closing would claim someone dealt with it when nobody did.
  //
  // Best-effort: a notification that will not close must not abort a run that
  // is about to do real work. resolveTask never throws by contract; the .catch
  // is belt-and-braces.
  await esignAlertService
    .resolveTask(db, { dedupeKey: 'reconcile-provider' })
    .catch((e) => console.error(`[ESIGN RECONCILE] provider-alert auto-resolve failed: ${e && e.message}`));
  ...
  const actorMoves = summary.changes.filter((c) => WEBHOOK_CARRIED_TRANSITIONS.has(c.to));
  summary.actor_moved = actorMoves.length;

  if (!dryRun && actorMoves.length >= 1) {
    try {
      const { at: lastSeen, via } = await lastProvenDelivery(db);
      const staleMs = WEBHOOK_SILENCE_HOURS * 60 * 60 * 1000;
      const silent = !lastSeen || (Date.now() - lastSeen.getTime()) > staleMs;

      if (silent) {
        const shown = actorMoves.slice(0, 5)
          .map((c) => `  • ${c.tracking_id}: ${c.from} → ${c.to}`).join('\n');
        const more = actorMoves.length > 5
          ? `\n  …and ${actorMoves.length - 5} more (see the job result).` : '';
        const since = lastSeen
          ? `${lastSeen.toISOString()}${via === 'audit' ? ' (from the event log — no heartbeat on record)' : ''}`
          : 'EVER (no delivery on record)';

        const alert = await esignAlertService.raiseTask(db, {
          // A STANDING CONDITION, not an event: this job runs nightly, so an
          // outage lasting a week would otherwise produce seven identical open
          // tasks. Keyed, so the second and later nights touch the first task
          // instead. routes/api.esign.js clears it when a delivery gets in.
          // Literal rather than esignAlertService.DEDUPE_KEYS.WEBHOOK_DOWN
          // because this module is exercised against a mock of that service —
          // keep the two in sync.
          dedupeKey: 'webhook-down',
          title: `E-sign webhooks appear to be DOWN`,
          desc:
            `Tonight's reconciliation applied ${actorMoves.length} status change(s) that a ` +
            `webhook should have delivered first, and no delivery has been accepted since ` +
            `${since}.\n\n` +
            `${shown}${more}\n\n` +
            ...
            `immediately.\n\nCheck, in this order:\n` +
            `  • Cloud Run logs, filter [ESIGN WEBHOOK]. NOTHING at all = Zoho stopped calling;\n` +
            `    confirm the webhook is still enabled in the Zoho Sign console.\n` +
            `  • 'rejected delivery … token_mismatch' = Zoho's URL token vs esign_webhook_token.\n` +
            `  • 'hmac REJECTED' = esign_webhook_hmac_mode 'enforce' vs esign_webhook_secret.`,
        });
        summary.webhook_silence_alerted = Boolean(alert.ok);
        console.warn(
          `[ESIGN RECONCILE] ${actorMoves.length} actor-driven move(s) with the webhook channel ` +
          `silent since ${lastSeen ? lastSeen.toISOString() : 'ever'}` +
          `${via === 'audit' ? ' (audit fallback)' : ''} — dead-channel task ` +
        ...
        if (filing.filed && !filing.warnings.length) {
          ...
          await esignService.appendEvent(db, row.id, {
            ...
            payload: {
              ...
              placement: filing.placement || null, source: 'reconcile', late: true,
            },
          }).catch(() => {});
        } else if (filing.filed) {
          // Filed, WITH warnings — the fallback ladder placed it in the
          // unsorted folder, or auto-created the case folder, or filed a ZIP.
          // That is a success needing a follow-up, not a failure: counting it
          // in the nightly problem summary would print "filing failed" about
          // a document that is sitting in Dropbox. Reuse the webhook path's
          // announcer so the per-request task (with the direct link and the
          // move-it instruction) is identical however the filing arrived.
          summary.refiled += 1;
          console.log(
            `[ESIGN RECONCILE] late-filed request ${row.id} → ${filing.signedPdfPath} ` +
            `(${filing.placement || 'case'}, ${filing.warnings.length} warning(s))`
          );
          await esignWebhookService._announceFiling(db, row, {
            filing, source: 'reconcile', late: true, alerted: false,
          }).catch((e) => console.error(`[ESIGN RECONCILE] announce failed for request ${row.id}: ${e.message}`));
  ...
  if (summary.failed > 0 && !dryRun) {
    ...
    await esignAlertService.raiseTask(db, {
      // The count in the title MOVES between runs ("6 problem(s)" vs "2
      // problem(s)") while the condition is the same one, which is precisely
      // why title equality could never have deduped this. Tasks 1082/1083/1084
      // are three identical Pending copies from three consecutive nights.
      // Literal rather than DEDUPE_KEYS.RECONCILE_FAILURES — see above.
      dedupeKey: 'reconcile-failures',
  ...
  params: [
    { name: 'template_id', type: 'integer', required: true, min: 1, placeholderAllowed: true,
      description: 'contract_templates.id — pick from the template admin list, or a ' +
        '{{placeholder}} for a dynamic template (fields then can\'t be pre-loaded in the editor).',
      example: 3 },
