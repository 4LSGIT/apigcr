 ...
 *   transparency  {string?}  — 'opaque' (default, blocks busy time) | 'transparent' (shows Free)
...
fns.gcal_create_event = async (params, db) => {
    ...
    const { summary, start, end, description, location, attendees,
            send_updates, credential_id, calendar_id, transparency } = params;
...
fns.gcal_create_event.__meta = {
  ...
  params: [
    ...
    { name: 'transparency', type: 'enum', required: false, enum: ['opaque', 'transparent'],
      description: "'transparent' shows the event as Free — visible but does not block busy time (GCal or scheduler freeBusy)." },
 ...
 *   transparency  {string?}  — 'opaque' (blocks busy time) | 'transparent' (shows Free)
 *   credential_id {number?}  — override
 *   calendar_id   {string?}  — override
 *
 * example config:
...
fns.gcal_update_event.__meta = {
  ...
  requiredWith: [['summary', 'start', 'end', 'description', 'location', 'attendees', 'transparency']],
