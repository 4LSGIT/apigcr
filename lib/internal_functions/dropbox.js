...
// Ceiling for dropbox_save_url's poll wait. A workflow step's wait is spent
// INSIDE the advance request (workflow_resume runs under /process-jobs, whose
// promises are awaited before the response), so this needs an in-request
// ceiling. 300s is the house per-step budget already asserted by
// lib/versionDiff.js RETRY_LADDER_BUDGET_SEC and by the batch note at
// routes/process_jobs.js:348, which sizes it against the service's 900s
// timeoutSeconds. NOT the webhook cap (120s) — that one bounds a held socket,
// this bounds a 1.5s poll loop.
//
// Raising this further means revisiting Cloud Run timeoutSeconds first: with
// error_policy retry_then_abort/max_retries:1 a thrown mid-poll error costs
// two full waits plus backoff, so 300s here is already ~610s of worst case.
//
// Clamped at runtime as well as declared in __meta, matching job_executor.js's
// CUSTOM_CODE_MAX_TIMEOUT_MS handling: __meta catches a bad literal at save
// time, the clamp catches anything that arrives some other way.
const SAVE_URL_MAX_TIMEOUT_MS = 300000;
 ...
 *
 * Waits for completion by default (25s, tunable via timeout_ms); if still
 * running, returns { status: 'in_progress', async_job_id } instead of failing.
 * NOTE: an in_progress result carries NO metadata, so callers reading
 * {{this.output.metadata.path_display}} must gate on status === 'complete'.
 *
 * params:
 ...
 *   timeout_ms    {number?}  — poll ceiling, default 25000, max 300000
 *   credential_id {number?}
 *
 * example config:
 *   {
...
fns.dropbox_save_url = async (params, db) => {
    const dropbox = require('../../services/dropboxService');
    const { url, path, shared_link, filename, subfolder, wait, credential_id,
            case_id, contact_id, timeout_ms } = params;
    ...
    // Non-positive / NaN → undefined, so saveUrl falls back to its own
    // SAVE_URL_TIMEOUT_MS default (25s) exactly as before this param existed.
    const rawTimeout = Number(timeout_ms);
    const timeoutMs = Number.isFinite(rawTimeout) && rawTimeout > 0
      ? Math.min(Math.floor(rawTimeout), SAVE_URL_MAX_TIMEOUT_MS)
      : undefined;

    console.log(`[DROPBOX_SAVE_URL] ${url} → ${path ? `"${path}"` : `link+${filename}`}` +
                `${timeoutMs ? ` (wait ≤ ${timeoutMs}ms)` : ''}`);
    ...
    //
    // ALWAYS register; link ONLY when the step was told whose file this is.
    //
    // Registration needs nothing but the metadata Dropbox already returned, so
    // it is free and it makes an automation-written file visible in the
    // documents list immediately instead of after the next delta. Attribution
    // is the opposite: `shared_link` is usually "{{case_dropbox}}", which
    // resolves to a case FOLDER and would let this step INFER a case — and an
    // inference is exactly what the path-based sync already does, better and
    // with a reconciler behind it. Guessing here would write an 'upload'-relation
    // link (which no reconciler will ever retract) off a guess. So the link is
    // made only from case_id / contact_id, which a workflow author states
    // explicitly, and left to the sweep otherwise.
    //
    // ONLY ON 'complete'. An in_progress save has no metadata yet — Dropbox is
    // still fetching — and there is nothing to register. The delta covers it.
    if (result && result.status === 'complete' && result.metadata) {
      const ingest = require('../../services/documentIngestService');
      const links = [];
      if (case_id    != null && case_id    !== '') links.push({ type: 'case',    id: case_id });
      if (contact_id != null && contact_id !== '') links.push({ type: 'contact', id: contact_id });
      // Safe variant: a workflow step must not fail because bookkeeping did.
      await ingest.registerWrittenSafe(db, {
        entry:       result.metadata,
        links,
        createdBy:   null,             // automations have no user
...
fns.dropbox_save_url.__meta = {
  ...
  params: [
    ...
    { name: 'wait', type: 'boolean', required: false, default: true,
      description: 'Poll until complete before returning (see timeout_ms).' },
    { name: 'timeout_ms', type: 'integer', required: false, default: 25000,
      min: 1000, max: SAVE_URL_MAX_TIMEOUT_MS,
      description:
        'How long to poll before giving up and returning status "in_progress" ' +
        '(default 25000). Raise it for slow sources — a server-rendered PDF can ' +
        'outrun 25s. The wait is spent inside the workflow advance request, so ' +
        `the ceiling is ${SAVE_URL_MAX_TIMEOUT_MS}ms. Ignored when wait is false.` },
    { name: 'case_id', type: 'string', required: false, placeholderAllowed: true,
      description: 'Attribute the saved file to this case in the documents registry. ' +
                   'Omit and the file is still registered — it just waits for the ' +
                   'path-based sweep to work out whose it is.' },
    { name: 'contact_id', type: 'string', required: false, placeholderAllowed: true,
      description: 'Attribute the saved file to this contact in the documents registry. ' +
                   'Contacts have no Dropbox folder convention, so this is the ONLY way ' +
                   'a saved file is ever linked to one.' },
    { name: 'credential_id', type: 'integer', required: false },
  ],
  exclusiveOneOf: [['path', 'shared_link']],
  example: { url: '{{attachment_url}}', shared_link: '{{case_dropbox}}', subfolder: 'Client Uploads', filename: ' {{contact_name}} - statement.pdf', case_id: '{{cases.case_id}}' }
};
