...
fns.wait_for.__meta = {
  category: 'timing',
  workflowOnly: true,
  // controlFlow drives workflow_engine.js isControlStep(). Load-bearing here:
  // the skip-block paths above return next_step with NO delayed_until, so
  // without this flag the engine ignores them and sequential-advances.
  controlFlow: true,
...
// ─────────────────────────────────────────────────────────────
// business_deadline — Shabbos/Yom-Tov-aware deadline computation.
//
// Computes "N days from now (floor), landing on the next unrestricted
// business day at HH:mm firm time" via calendarService.nextBusinessDay, and
// returns it BOTH as an ISO instant and as a duration-from-now string that
// request_decision's `timeout` param accepts directly (ms() parses "4020m").
//
// The point (website-lead routing arc, 2026-09): a flat 48h HITL timeout on
// a Thursday-evening lead escalates Shabbos afternoon and lands stale
// Monday. Feeding {{intake_deadline_duration}} from this step instead makes
// the timeout land on a morning someone is actually working.
//
// NOT a control step (no next_step, no delayed_until) — it only computes.
// Lives in timing.js for cohesion; the control functions around it are
// marked via __meta.controlFlow, which this one deliberately lacks.
// ─────────────────────────────────────────────────────────────
fns.business_deadline = async (params = {}) => {
  const calendarService = require('../../services/calendarService'); // lazy (convention)

  // days — calendar-day FLOOR before the business-day search starts
  // (nextBusinessDay's minDaysAhead). 0 = "today if the target time is
  // still ahead and the day is unrestricted".
  let days = params.days === undefined || params.days === null || params.days === ''
    ? 2 : Number(params.days);
  if (!Number.isInteger(days) || days < 0 || days > 60) {
    throw new Error('business_deadline: days must be an integer 0–60');
  }

  const time = String(params.time ?? '10:00').trim();
  if (!/^([01]?\d|2[0-3]):[0-5]\d$/.test(time)) {
    throw new Error('business_deadline: time must be HH:mm (24h, firm timezone)');
  }

  // from — optional anchor; naive strings resolve in FIRM_TZ via
  // parseUserDateTime (same contract as schedule_resume's resumeAt).
  let fromDate = new Date();
  if (params.from !== undefined && params.from !== null && String(params.from).trim() !== '') {
    const parsed = parseUserDateTime(String(params.from).trim());
    if (!parsed || isNaN(parsed.getTime())) {
      throw new Error(`business_deadline: could not parse from "${params.from}"`);
    }
    fromDate = parsed;
  }

  const at = await calendarService.nextBusinessDay(fromDate, {
    timeOfDay:    time,
    minDaysAhead: days,
  });

  // Duration from NOW (not from `from`) — this is what a timeout consumes.
  const minutes = Math.max(1, Math.ceil((at.getTime() - Date.now()) / 60000));

  return {
    success: true,
    output: {
      deadline_at: at.toISOString(),
      duration:    `${minutes}m`,
      minutes,
      days,
      time,
...
fns.business_deadline.__meta = {
  category: 'timing',
  description: 'Compute a Shabbos/Yom-Tov-aware deadline: at least `days` calendar days ahead, landing on the next unrestricted business day at `time` (firm timezone). Returns the instant (deadline_at, ISO UTC) and a duration-from-now string (duration, e.g. "4020m") that request_decision\'s timeout param accepts directly. Capture via set_vars and feed {{deadline_duration}} to the decision step so HITL timeouts never expire over Shabbos. Not a control step — computes only, never pauses.',
  params: [
    { name: 'days', type: 'integer', required: false, placeholderAllowed: true,
      description: 'Calendar-day floor before the business-day search (0–60, default 2). 0 = today if the target time is still ahead.',
      example: '2' },
    { name: 'time', type: 'string', required: false, placeholderAllowed: true,
      description: 'Target time of day, HH:mm 24h in the firm timezone (default "10:00").',
      example: '10:00' },
    { name: 'from', type: 'string', required: false, placeholderAllowed: true,
      description: 'Optional anchor instant (ISO, SQL-ish, or naive firm-local — same forms schedule_resume accepts). Default: now. The duration output is always measured from NOW regardless.',
    },
  ],
  example: { days: '2', time: '10:00' },
};
