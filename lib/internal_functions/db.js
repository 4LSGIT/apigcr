...
const QUERY_DB_ALLOWED_TABLES = new Set([
  ...
  'job_results',
  // NOTE: cases.case_judge / cases.case_trustee are varchar NAME columns on
  // cases. The `judges` and `trustees` tables were dropped (slice 7B / m6,
  // 2026-09-23); the canonical judge/trustee link is the *_contact_id twin →
  // contacts (+ contact_roles), and the trustee roster is served by
  // lib/trusteeRoster.
]);

// cases.custom / contacts.custom (custom-fields S2): the raw JSON bag never
// travels whole — here that means never into workflow variables / step logs.
// Stripped by name after the query like the rest, so `*` is covered too.
 ...
 *
 * JOIN shape:
 *   { type: "left", table: "case_relate", alias: "cr",
 *     on: { left: "cases.case_id", right: "cr.case_relate_case_id" } }
 *
 * WHERE shape:
 ...
 *
 * Example — fetch a case row with its related client contact:
 *   {
 *     "function_name": "query_db",
 *     "params": {
 *       "select": ["cases.case_number", "cases.case_judge", "cases.case_trustee",
 *                  "c.contact_name", "c.contact_email"],
 *       "from": "cases",
 *       "join": [
 *         { "type": "left", "table": "case_relate", "alias": "cr",
 *           "on": { "left": "cases.case_id", "right": "cr.case_relate_case_id" } },
 *         { "type": "left", "table": "contacts",    "alias": "c",
 *           "on": { "left": "cr.case_relate_client_id", "right": "c.contact_id" } }
 *       ],
...
const WRITE_POLICY = {
  ...
  // `kind` is immutable after creation (S1). Flipping it here would produce a
  // note holding checkitems, or a checklist holding a body — data that
  // lib/checklistStatus.js refuses to touch and that neither renderer in
  // public/checklistView.html will display. PATCH /checklists/:id 400s on it
  // outright; this closes the same hole on the generic surface.
  //
  // NOT blocked: `body` (an automation writing into a note is the obvious
  // future use) and `status` (writable today and overwritten by the next
  // recompute — pre-existing posture, a separate decision).
  //
  // NOTE that _wdbValidateSet is shared by update_db AND insert_db, so this
  // also stops insert_db creating a note; a generic INSERT gets the column
  // default 'checklist'. Notes are created through POST /checklists, which is
  // the route that enforces body/items exclusivity.
  checklists:        { where: ['id', 'link', 'link_type'],                                         block: ['kind'], insert: true },
