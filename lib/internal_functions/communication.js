...
fns.send_email = async (params, db) => {
    const { from, to, subject, text, html, attachment_urls, attachment_names, include_signature } = params;
    ...
    // Lenient truthiness: workflow step configs and resolved {{placeholders}}
    // deliver booleans as strings. Anything else (undefined, '', 'false', 0)
    // → no signature, matching pre-feature behavior.
    const wantSig = include_signature === true || include_signature === 1 ||
      /^(true|1|yes)$/i.test(String(include_signature ?? ''));

    console.log(`[SEND_EMAIL] from=${from} to=${to} subject="${subject}"${attachment_urls ? ' (with attachments)' : ''}${wantSig ? ' (signature)' : ''}`);
    const result = await emailService.sendEmail(db, {
      from, to, subject, text, html,
      ...(attachment_urls  && { attachment_urls }),
      ...(attachment_names && { attachment_names }),
      ...(wantSig          && { signature: true }),
    });
...
fns.send_email.__meta = {
  ...
  params: [
    ...
    { name: 'include_signature', type: 'boolean', required: false, default: false,
      description:
        'Append the sender\'s stored signature (email_credentials.signature_html' +
        '/_text) after the body. No-op when the from-address has no signature ' +
        'configured. Accepts true/"true"/"1"/"yes".' },
  ],
