...
// step editor. It carries a MINIMAL __meta (uiHidden, params: []) whose sole
// purpose is to remove it from the editor pickers — it reads raw envelope
// dot-paths supplied via the rule's params_mapping, which only exist in the
// ingest pipeline, so a param form would be meaningless. params: [] is safe:
// __validateParamsAgainstMeta only checks DECLARED params and groups; it never
// rejects undeclared incoming params, so the ingest-supplied envelope params
// always validate.
//
...
fns.court_extract.__meta = {
  category: 'system',
  uiHidden: true,
  description: 'Court NEF extraction pipeline entry point. Invoked by the '
    + 'court email pipeline (email-ingest internal_function action, envelope '
    + 'passthrough), not authored as a workflow/sequence step. Params are '
    + 'documented (not left empty) so the meta-driven form renderer stays '
    + 'accurate for any pre-existing step — an empty params list would render '
    + 'an empty form and wipe a saved step\'s params on save. All optional; '
    + 'the implementation tolerates a missing envelope.',
  params: [
    { name: 'message_id', type: 'string', required: false, placeholderAllowed: true,
      description: 'Canonical id (headers.message_id). Angle brackets stripped; -test- marker forces dry-run downstream.' },
    { name: 'exim_message_id', type: 'string', required: false, placeholderAllowed: true,
      description: 'Fallback id source (envelope.exim_message_id) when message_id is absent.' },
    { name: 'subject', type: 'string', required: false, placeholderAllowed: true,
      description: 'Email subject. Attacker-influenceable — rides inside untrusted_user_input in the AI call.' },
    { name: 'from_email', type: 'string', required: false, placeholderAllowed: true,
      description: 'Sender address (from.email).' },
    { name: 'body', type: 'string', required: false, placeholderAllowed: true, multiline: true,
      description: 'Plain-text body (envelope text). Preferred body source.' },
    { name: 'body_html', type: 'string', required: false, placeholderAllowed: true, multiline: true,
      description: 'HTML body fallback, used only when body is empty.' },
  ],
  example: { message_id: '{{headers.message_id}}', subject: '{{subject}}',
             from_email: '{{from.email}}', body: '{{text}}', body_html: '{{html}}' }
};
...
fns.court_review_retry = async ({ limit = 100, dry_run = false } = {}, db) => {
    ...
    for (const row of rows) {
      ...
      try {
        const rr = await courtRerun.rerunCalRow(db, row, { allowExtract: false, source: 'court_sweep' });
...
// 4-section HTML email (Actioned / Covered Elsewhere / Needs Review /
// Ignored–No Action) and sends it via emailService. The Ignored section lists EVERY no-action subject in
...
// Dedup-class skip reasons emitted by courtExecutor's natural-key guards. A
// none-row whose skipped_json entries are ALL from this set means "parsed
// correctly, the target record already existed" — the executor found a
// colliding appointment/event (usually created seconds earlier by an external
// automation) or a case field already carried the value. Review-class skips
// (event_missing_date, no_primary_contact, …) also push reviewReasons, so
// those rows become outcome='queued' and never reach this partition.
const DEDUP_SKIP_REASONS = new Set([
  'appt_exists', 'event_exists', 'event_slot_exists', 'noop', 'fill_only_occupied',
]);

// skipped_json / actions_json (JSON columns) → parsed array|null. mysql2
// returns JSON columns already parsed; the readonly HTTP API too. Guard the
// string case defensively (same convention as summarizeCourtActions).
function _jsonArr(v) {
  let a = v;
...
// Row is "covered elsewhere" iff outcome none AND skipped_json is a non-empty
// array AND every entry's reason is dedup-class.
function _isCoveredRow(r) {
  if (r.outcome !== 'none') return false;
  const sk = _jsonArr(r.skipped_json);
  if (!sk || sk.length === 0) return false;
  return sk.every((e) => e && DEDUP_SKIP_REASONS.has(e.reason));
}

// Render "what was already in place" for a covered row: join each skipped
// entry to its proposed action via action_index into actions_json, format the
// parsed date/time with Luxon in firmTz ("Jul 16, 8:30 AM"; date-only when no
// time). Multiple entries joined with '; '. Plain text — caller escapes.
function _coveredNote(r, firmTz) {
  const { DateTime } = require('luxon');
  const sk = _jsonArr(r.skipped_json) || [];
  const actions = _jsonArr(r.actions_json) || [];
  const fmtDT = (date, time) => {
    if (!date) return '(no date)';
    try {
      if (time) {
        const dt = DateTime.fromISO(`${date}T${time}`, { zone: firmTz });
        if (dt.isValid) return dt.toFormat('LLL d, h:mm a');
      }
      const d = DateTime.fromISO(String(date), { zone: firmTz });
      if (d.isValid) return d.toFormat('LLL d');
    } catch { /* fall through */ }
    return String(date) + (time ? ' ' + time : '');
  };
  const parts = sk.map((e) => {
    const act = (e && Number.isInteger(e.action_index)) ? actions[e.action_index] : null;
    const f = (act && act.fields) || {};
    switch (e && e.reason) {
      case 'appt_exists':
        return `parsed ${f.appt_type || 'appointment'} @ ${fmtDT(f.date, f.time)}` +
               ` — already scheduled (appt ${e.appt_id ?? '?'})`;
      case 'event_exists':
      case 'event_slot_exists':
        return `parsed event: ${f.event_type || f.event_title || 'event'} @ ${fmtDT(f.date, f.time)}` +
               ` — already on calendar (event ${e.event_id ?? '?'})`;
      case 'noop':
      case 'fill_only_occupied':
        return `case field ${e.field || '?'} — already set`;
      default:
        return String((e && e.reason) || 'skipped');
    }
  });
  return parts.join('; ');
}

// Build the coverage digest (4 sections: Actioned / Covered Elsewhere / Needs
// Review / Ignored). Pure: takes the window rows (newest-first) + opts
// {days, firmTz}; returns { html, counts }. Inline styles only (email
...
function buildCourtSummaryHtml(rows, opts = {}) {
  ...
  const covered     = list.filter((r) => _isCoveredRow(r)); // none + all-dedup skips
  const needsReview = list.filter((r) => r.outcome === 'queued');
  const ignoredAll  = list.filter((r) =>
    (r.outcome === 'none' && !_isCoveredRow(r)) || r.outcome === 'error'); // newest-first across both
  const noneCount   = ignoredAll.filter((r) => r.outcome === 'none').length;
  const errorCount  = ignoredAll.filter((r) => r.outcome === 'error').length;
  ...
  // ---- §A2 COVERED ELSEWHERE (parsed correctly; target record already existed) ----
  const rowsCov = covered.map((r) =>
    '<tr>' +
    td(fmtDate(r.created_at), 'white-space:nowrap;color:#6b7280;width:64px;') +
    td(drTag(r) + esc(r.subject || '(no subject)'), 'font-weight:600;') +
    td(caseLabel(r), 'color:#374151;') +
    td(esc(_coveredNote(r, firmTz)), 'color:#0e7490;') +
    '</tr>');
  const secCov = sectionH('Covered Elsewhere', '#0891b2', covered.length) +
    '<p style="font-size:12px;color:#6b7280;margin:2px 0 4px;">Parsed correctly — the target ' +
    'record already existed (e.g. created by another automation). Nothing was missed.</p>' +
    table(['Date', 'Subject', 'Case', 'What was already in place'], rowsCov, '#0891b2');
  ...
  const rowsC = ignoredAll.map((r) => {
    let thought;
    const skArr = _jsonArr(r.skipped_json);
    const actArr = _jsonArr(r.actions_json);
    ...
    // Legacy fallback: a pre-upgrade none-row (no skipped_json persisted) that
    // DID propose actions — we can't tell dedup-skip from real ignore, say so.
    else if (r.outcome === 'none' && (!skArr || skArr.length === 0) && actArr && actArr.length > 0)
      thought = (r.classification || '(unclassified)') +
        ' — actions proposed, skip detail unavailable (pre-upgrade row)';
  ...
  const tally = `<div style="font-size:14px;margin:10px 0 6px;color:#374151;">` +
    ...
    `<strong>${covered.length}</strong> covered elsewhere, ` +
...
fns.court_activity_summary = async (
  ...
  // cfg() = email_automations setting with AUTO_EMAIL env fallback.
  const fromAddr = from
  ...
  const itAddr = cfg('email_it') || 'it@4lsg.com';
...
fns.court_activity_summary.__meta = {
  category: 'system',
  description:
    'Coverage-review digest of court_ai_log over a rolling window. Emails a 4-section ' +
    'HTML summary (Actioned / Covered Elsewhere / Needs Review / Ignored–No Action); the Ignored section lists ' +
