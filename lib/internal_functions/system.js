 ...
 *
 * SPLIT-PHASE DISPATCH (2026-08-30): also sweeps domain_event_queue, the
 * work queue that FEEDS these executions. Same job, same batching — it needs
 * no scheduling of its own, and a second daily sweep job for a sibling table
 * would only be a thing to forget about. The queue needs retention MORE than
 * the audit log does: no_rules executions are capped at 20 rows per event
 * type (_insertNoRulesCapped), so a high-volume no-rules event type writes 20
 * audit rows in total but one queue row EVERY time. The queue therefore grows
 * strictly faster than the table it feeds.
 *
 * Terminal rows only — 'pending' and 'running' are live work and are never
 * deleted here regardless of age. A stuck row is lib/domainEventDrain.js's
 * sweepStale problem; deleting it would be silent loss.
 *
 * params:
 ...
 *   keep_queue_days    {number?} — retention for done/error domain_event_queue
 *                                  rows (default 14). Shorter than the audit
 *                                  windows on purpose: this is a work queue,
 *                                  not history. trigger_executions is the
 *                                  audit record and keeps its own envelope
 *                                  copy, so a swept queue row loses nothing
 *                                  that is not still on file next to it.
 */
fns.sweep_trigger_executions = async (params = {}, db) => {
  ...
  const keepQueueDays   = Number(params.keep_queue_days)   > 0 ? Number(params.keep_queue_days)   : 14;
  const BATCH = 5000;
  let deleted = 0;
  for (let i = 0; i < 20; i++) {
  ...
  // Queue retention. Must never break execution retention above, so it owns
  // its errors — a failed queue sweep costs a day of extra rows, and the
  // deleted counter above is already logged.
  let queueDeleted = 0;
  try {
    for (let i = 0; i < 20; i++) {
      const [res] = await db.query(
        `DELETE FROM domain_event_queue
          WHERE status IN ('done','error')
            AND created_at < DATE_SUB(NOW(), INTERVAL ? DAY)
          LIMIT ${BATCH}`,
        [keepQueueDays]
      );
      queueDeleted += res.affectedRows;
      if (res.affectedRows < BATCH) break;
    }
    console.log(`[TRIGGER SWEEP] domain_event_queue deleted=${queueDeleted} keep_queue_days=${keepQueueDays}`);
  } catch (err) {
    console.error('[TRIGGER SWEEP] domain_event_queue sweep failed:', err.message);
  }
  ...
  return {
    success: true,
    output: {
      deleted, stranded, queue_deleted: queueDeleted,
      keep_days: keepDays, keep_matched_days: keepMatchedDays, keep_queue_days: keepQueueDays,
    },
  };
};
...
fns.sweep_trigger_executions.__meta = {
  category: 'system',
  description: 'Retention sweep for trigger_executions: no_match/no_rules rows kept keep_days (default 30), matched/error rows kept keep_matched_days (default 90). Also sweeps terminal (done/error) domain_event_queue rows kept keep_queue_days (default 14) — pending/running rows are live work and are never deleted. Batched deletes. Also alerts on executions stranded mid-dispatch for over an hour (suspended post-response work).',
  params: [
    ...
    { name: 'keep_queue_days', type: 'integer', required: false, default: 14,
      description: 'Days to keep done / error domain_event_queue rows. Pending and running rows are never swept.' },
  ],
  example: {}
};

/**
 * emit_stage_aged — nightly emitter for the case.stage_aged synthetic event.
 *
 * Absence is inexpressible in a mutation-driven trigger system ("docs still
 * incomplete 7 days after intake"). This function turns stage AGE into an
 * event: for each piped case's CURRENT stage (latest case_stage_log row),
 * and for each threshold in the ladder, it claims (stage_log_id, threshold)
 * in case_stage_aged_emitted and emits case.stage_aged — exactly once per
 * stage entry per threshold. Re-entering a stage creates a new log row, so
 * its nudges re-arm for free.
 *
 * FORWARD-LOOKING ONLY (the grace window): a crossing fires only while
 *   days_in_stage >= threshold AND days_in_stage < threshold + grace_days.
 * Without it, a case backfilled with a historical entered_at at 240 days in
 * stage would claim and fire every rung at once — and gradual backfill would
 * turn that into an indefinite trickle of stampedes. Cases imported
 * already-stale are structurally invisible to this event; that triage is a
 * one-time report's job, not this event's.
 *
 * "Latest row" is ordered (entered_at DESC, id DESC) — the SAME ordering
 * pipelineService uses — NOT MAX(id): an appended backfill row can carry a
 * historical entered_at with a later id.
 *
 * Claim-then-emit is deliberate at-most-once: a crash in the ms between
 * claim and emit loses one nudge (a human can recover); emit-then-claim
 * risks duplicate SMS / stage moves, which is worse.
 *
 * Emissions are AWAITED (not fire-and-forget) so dispatch completes while
 * the job holds CPU instead of being orphaned into Cloud Run's throttled
 * post-response time. Each emission is its own ROOT event (job-executor
 * context has no ALS scope → emit() opens a fresh root per call), so the
 * engine's 50-dispatch budget is per emission — only max_emissions bounds
 * the run. Hitting the cap alerts and stops BEFORE claiming, so unemitted
 * crossings stay claimable next night while inside their grace window.
 *
 * Days are whole 24-hour periods (TIMESTAMPDIFF(DAY, …)), computed BY THE
 * DATABASE — the pool's fake-UTC Dates make JS date math wrong off-UTC.
 *
 * params:
 *   thresholds    {string|number[]?} — csv or array of day rungs (default 3,7,14,30,60)
 *   grace_days    {number?}          — recent-crossing window (default 7)
 *   max_emissions {number?}          — per-run cap (default 200)
 *   dry_run       {boolean?}         — list would-emit crossings; no claims, no emits
 */
fns.emit_stage_aged = async (params = {}, db) => {
  const domainEvents = require('../domainEvents'); // deferred require (circular dep safety)

  let thresholds = params.thresholds ?? [3, 7, 14, 30, 60];
  if (typeof thresholds === 'string') thresholds = thresholds.split(',');
  if (!Array.isArray(thresholds)) {
    throw new Error('emit_stage_aged: thresholds must be an array or comma-separated string');
  }
  thresholds = [...new Set(
    thresholds.map(t => parseInt(t, 10)).filter(t => Number.isInteger(t) && t > 0)
  )].sort((a, b) => a - b);
  if (!thresholds.length) {
    throw new Error('emit_stage_aged: thresholds resolved to an empty list');
  }

  const graceDays    = Number(params.grace_days)    > 0 ? Math.floor(Number(params.grace_days))    : 7;
  const maxEmissions = Number(params.max_emissions) > 0 ? Math.floor(Number(params.max_emissions)) : 200;
  // Review F5 (stage_aged round): max_emissions is a COUNT, but the guard it
  // lives under is TIME — process_jobs' recoverStuckJobs flips a job still
  // running after RECOVERY_WINDOW_MIN=15 back to pending, spawning a second
  // concurrent run (harmless under INSERT IGNORE, but it double-advances the
  // recurrence). A wall-clock bound keeps one run comfortably inside that
  // window regardless of how slow individual dispatches turn out to be.
  const maxRuntimeMs = Number(params.max_runtime_ms) > 0 ? Math.floor(Number(params.max_runtime_ms)) : 8 * 60 * 1000;
  const dryRun       = !!params.dry_run;
  const startedAt    = Date.now();

  // Candidates: current stage per piped case. Terminal stages and
  // Closed/Concluded cases excluded; a log row whose stage_id no longer
  // resolves (LEFT JOIN miss) is treated as non-terminal and still ages.
  // Ordered oldest-first so near-window-expiry crossings win under the cap.
  const [candidates] = await db.query(
    `SELECT x.id AS stage_log_id, x.case_id, x.template_id, x.stage_id,
            x.stage_key, x.case_stage, x.status_label, x.entered_at,
            TIMESTAMPDIFF(DAY, x.entered_at, NOW()) AS days_in_stage,
            c.case_type, c.case_subtype
       FROM (
              SELECT l.*,
                     ROW_NUMBER() OVER (PARTITION BY l.case_id
                                        ORDER BY l.entered_at DESC, l.id DESC) AS rn
                FROM case_stage_log l
            ) x
       JOIN cases c ON c.case_id = x.case_id
       LEFT JOIN pipeline_stages ps ON ps.id = x.stage_id
      WHERE x.rn = 1
        AND COALESCE(ps.is_terminal, 0) = 0
        AND c.case_stage NOT IN ('Closed', 'Concluded')
        AND TIMESTAMPDIFF(DAY, x.entered_at, NOW()) >= ?
      ORDER BY x.entered_at ASC, x.id ASC`,
    [thresholds[0]]
  );

  let emitted = 0, duplicates = 0, capped = false, timedOut = false;
  const wouldEmit = [];

  outer:
  for (const row of candidates) {
    const days = Number(row.days_in_stage);
    for (const t of thresholds) {
      // Grace window: fire only on RECENT crossings. Ladder spacing
      // [3,7,14,30,60] with a 7-day window means at most rungs 3 and 7 can
      // both be in-window for one case (days 7–9) — bounded catch-up after
      // an outage or first arm, never a stampede.
      if (!(days >= t && days < t + graceDays)) continue;

      if (dryRun) {
        wouldEmit.push({
          case_id: row.case_id, stage_key: row.stage_key,
          stage_log_id: row.stage_log_id, threshold_days: t, days_in_stage: days,
        });
        continue;
      }

      if (emitted >= maxEmissions) { capped = true; break outer; }               // stop BEFORE claiming
      if (Date.now() - startedAt >= maxRuntimeMs) { timedOut = true; break outer; } // ditto — time bound

      const [res] = await db.query(
        `INSERT IGNORE INTO case_stage_aged_emitted
           (stage_log_id, threshold_days, case_id)
         VALUES (?, ?, ?)`,
        [row.stage_log_id, t, row.case_id]
      );
      if (res.affectedRows !== 1) { duplicates++; continue; } // already claimed

      // emit() never throws; awaited so dispatch runs on-CPU (see header).
      await domainEvents.emit(db, 'case.stage_aged', {
        ...
        extra: { stage_log_id: row.stage_log_id },
      });
      emitted++;
    }
  }
  ...
  if (capped || timedOut) {
    try {
      const { alert } = require('../alerting');
      const bound = capped
        ? `per-run cap (${maxEmissions} emissions)`
        : `wall-clock bound (${maxRuntimeMs} ms — kept under process_jobs' 15-min stuck-job recovery)`;
      await alert(db, {
        source: 'app', kind: 'trigger_stage_aged_cap', severity: 'warning',
        group_key: 'trigger_stage_aged_cap',
        title: `emit_stage_aged stopped at its ${capped ? 'emission cap' : 'time bound'}`,
        message:
          `Hit ${bound}. Unemitted crossings stay unclaimed and retry tomorrow while ` +
          `inside their ${graceDays}-day grace window. A bound hit usually means a ` +
          `config change or backfill surprise — inspect before raising limits.`,
        context: { emitted, candidates: candidates.length, thresholds,
                   grace_days: graceDays, elapsed_ms: Date.now() - startedAt },
      }).catch(() => {});
    } catch (_) { /* alerting unavailable — counters still returned */ }
  }

  console.log(
    `[STAGE AGED] scanned=${candidates.length} emitted=${emitted} ` +
    `duplicates=${duplicates} capped=${capped} timed_out=${timedOut}${dryRun ? ' (dry run)' : ''}`
  );

  const output = {
    scanned: candidates.length, emitted, duplicates, capped,
    timed_out: timedOut, elapsed_ms: Date.now() - startedAt,
    thresholds, grace_days: graceDays, max_emissions: maxEmissions,
  };
  if (dryRun) { output.dry_run = true; output.would_emit = wouldEmit; }
  return { success: true, output };
};

fns.emit_stage_aged.__meta = {
  category: 'system',
  description:
    'Nightly emitter for the case.stage_aged synthetic trigger event. For each piped ' +
    'case\'s CURRENT pipeline stage, claims (stage_log_id, threshold) in ' +
    'case_stage_aged_emitted and emits case.stage_aged for every ladder rung whose ' +
    'crossing is RECENT (days_in_stage >= threshold and < threshold + grace_days). ' +
    'Exactly-once per stage entry per threshold; re-entering a stage re-arms its rungs. ' +
    'Forward-looking only — already-stale backfilled cases never fire. Terminal stages ' +
    'and Closed/Concluded cases skipped. Emissions awaited; each is its own root event. ' +
    'Days are whole 24-hour periods.',
  params: [
    { name: 'thresholds', type: 'string', required: false, default: '3,7,14,30,60',
      description: 'Comma-separated day rungs (an array also works at runtime). Deduped, sorted, must all be integers > 0.' },
    { name: 'grace_days', type: 'integer', required: false, default: 7,
      description: 'Fire only when the crossing happened within this many days. Absorbs job outages; blocks backfill stampedes.' },
    { name: 'max_emissions', type: 'integer', required: false, default: 200,
      description: 'Per-run emission cap. Hitting it alerts once and stops before claiming, so the remainder retries next run while in-window.' },
    { name: 'max_runtime_ms', type: 'integer', required: false, default: 480000,
      description: 'Wall-clock bound (default 8 min — safely inside the job runner\'s 15-min stuck-job recovery). Stops before claiming, same retry semantics as the cap.' },
    { name: 'dry_run', type: 'boolean', required: false, default: false,
      description: 'List would-emit crossings (uncapped) without claiming or emitting.' },
  ],
  example: {}
};
