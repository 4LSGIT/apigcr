...
fns.emit_stage_aged = async (params = {}, db) => {
  ...
  const [candidates] = await db.query(
    `SELECT x.id AS stage_log_id, x.case_id, x.template_id, x.stage_id,
            x.stage_key, x.status_label, x.entered_at,
  ...
  let emitted = 0, duplicates = 0, capped = false;
  const wouldEmit = [];
  ...
  if (capped) {
    try {
      ...
      await alert(db, {
        ...
        title: `emit_stage_aged hit its per-run cap (${maxEmissions})`,
        message:
          `Unemitted crossings stay unclaimed and retry tomorrow while inside their ` +
          `${graceDays}-day grace window. A cap hit usually means a config change or ` +
          `backfill surprise — inspect before raising max_emissions.`,
        context: { emitted, candidates: candidates.length, thresholds, grace_days: graceDays },
      }).catch(() => {});
  ...
  console.log(
    ...
    `duplicates=${duplicates} capped=${capped}${dryRun ? ' (dry run)' : ''}`
  );
