...
//                      'court email','docs','appt','update','task','event','esign')
...
// About-link (S2): the about-link target types are the FULL log_link_type
// enum — wider than LINK_TYPES, which excludes task/event because primary
// re-linking is a user action and those rows are machine-written. The
// about-link is a topical annotation; about=task/event are legitimate and
// surface via listLog's generic arm. Mirrors logService.ABOUT_TYPES.
const ABOUT_TYPES = [...LINK_TYPES, 'task', 'event'];
...
// About-link (S2): declared on BOTH create_log and phone_log — phone_log
// forwards its params object wholesale to logService.createLogEntry, so the
// about params flow through it identically, and keeping the two metas in
// lockstep is this file's stated convention (see EXTRA_PARAM / DATA_PARAM).
const ABOUT_TYPE_PARAM = {
  name: 'about_type', type: 'enum', required: false,
  enum: ABOUT_TYPES, placeholderAllowed: true,
  description:
    "Optional SECONDARY \"what it's about\" attribution (log_about_type), " +
    'independent of the primary identity link. Full log_link_type enum ' +
    '(incl. task/event — wider than update_log link_type; see set_log_about). ' +
    "Omit for no about-link ('none' is NOT accepted here — it is " +
    "set_log_about's clearing sentinel; on create, omitting the param is " +
    'the null state). Accepts a {{placeholder}}.'
};
const ABOUT_ID_PARAM = {
  name: 'about_id', type: 'string', required: false, placeholderAllowed: true,
  description:
    'Required non-blank when about_type is set. Entity ID for ' +
    'contact/case/appt/bill/task/event, OR the phone (normalized to 10 ' +
    "digits) / email (lowercased+trimmed) VALUE when about_type is " +
    'phone/email. Target existence is not validated (IDs are opaque).'
};
 ...
 *   about_type   {string|null}    — About-link S2: optional SECONDARY "what
 *                                   it's about" attribution; full
 *                                   log_link_type enum incl. task/event
 *   about_id     {string|number}  — required non-blank when about_type set;
 *                                   normalized like link_id (phone/email)
 ...
 *
 * Thin shim over services/logService.setLogAbout. Writes log_about_type /
 * log_about_id and NOTHING else — the primary identity link
 * (log_link_type / log_link_id / log_link) is untouched; that is
 * update_log's job. Motivating case: an email with outside counsel
 * (primary link = the counsel's email address) that concerns a client's
 * case — the about-link surfaces it on the case view without losing the
 * identity attribution.
 *
 * Unlike update_log, an UNLINK PATH EXISTS: about_type = 'none' clears
 * both columns (about_id is ignored). The about-link is a secondary
 * annotation, safe to clear; the primary link is not — the asymmetry is
 * deliberate.
 *
 * about_id normalization matches create_log/update_log link_id exactly,
 * via the same logService helpers:
 *   phone → normalized to 10 digits
 *   email → trimmed + lowercased
 *   contact/case/appt/bill/task/event → stored as-is (String)
 * An unusable value throws err.code = 'INVALID_LOG_ABOUT_ID'; a missing
 * row throws 'LOG_NOT_FOUND'.
 *
 * params:
 *   log_id      {number|string} — the log row (must exist)
 *   about_type  {string}        — 'contact','case','appt','bill','phone',
 *                                 'email','task','event', or 'none' to clear
 *   about_id    {string|number} — entity ID or phone/email VALUE; required
 *                                 unless about_type = 'none'
 *
 * example config:
 *   {
 *     "function_name": "set_log_about",
 *     "params": {
 *       "log_id": "{{logId}}",
 *       "about_type": "case",
 *       "about_id": "{{caseId}}"
...
fns.set_log_about = async (params, db) => {
    const logService = require('../../services/logService');
    const p = params || {};
    // 'none' → the service's unlink path (null). The sentinel exists because
    // enum params + {{placeholders}} don't express null cleanly in workflow
    // configs; the service itself also accepts null/'' directly.
    const about_type = p.about_type === 'none' ? null : p.about_type;
    const result = await logService.setLogAbout(db, {
      log_id: p.log_id,
      about_type,
      about_id: p.about_id
    });
...
fns.set_log_about.__meta = {
  category: 'log',
  description:
    'Set or clear the SECONDARY "what it\'s about" attribution ' +
    '(log_about_type / log_about_id) on an existing log entry. Never touches ' +
    'the primary identity link (update_log does that) or the log content. ' +
    "about_type 'none' clears the about-link.",
  params: [
    // Same placeholder-on-integer story as update_log.log_id: the canonical
    // caller resolves {{logId}} from a prior step, and the validator's
    // placeholder bypass runs before the type check.
    { name: 'log_id', type: 'integer', required: true, placeholderAllowed: true,
      description:
        'The log row. Must already exist — a missing row throws LOG_NOT_FOUND ' +
        'rather than creating one.' },
    { name: 'about_type', type: 'enum', required: true,
      enum: [...ABOUT_TYPES, 'none'], placeholderAllowed: true,
      description:
        "Full log_link_type enum (wider than update_log's link_type — " +
        'about=task/event are legitimate topical annotations), plus ' +
        "'none' to clear the about-link. Accepts a {{placeholder}}." },
    { name: 'about_id', type: 'string', required: false, placeholderAllowed: true,
      description:
        'Entity ID for contact/case/appt/bill/task/event, OR the phone ' +
        '(normalized to 10 digits) / email (lowercased+trimmed) VALUE when ' +
        "about_type is phone/email. Required unless about_type is 'none' " +
        '(then ignored). Target existence is not validated.' },
  ],
  example: { log_id: '{{logId}}', about_type: 'case', about_id: '{{caseId}}' },
};
