...
fns.documents_refresh_case_cache = async (params = {}, db) => {
  ...
  const out = await sync.refreshCaseFolderCache(db, { ...(limit ? { limit } : {}) });

  console.log(
    `[DOCUMENTS CACHE] scanned=${out.scanned} resolved=${out.resolved} ` +
    `failed=${out.failed} out_of_root=${out.out_of_root.length}`
  );
...
fns.documents_refresh_case_cache.__meta = {
  ...
  description:
    ...
    'whose documents would therefore never be registered. Fail-closed on app_settings ' +
    'documents_sync_enabled != "1".',
  params: [
    { name: 'limit', type: 'integer', required: false, default: 300,
      description: 'Cases to resolve this run, oldest-resolved first (never-resolved counts as oldest). ~150ms each, sequential.' },
  ],
  example: {}
};
