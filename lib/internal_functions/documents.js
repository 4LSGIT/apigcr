...
fns.documents_sync = async (params = {}, db) => {
  ...
  // SYNC-1 grace sweeper: promote pending deletes that survived the grace
  // window unseen. AFTER syncAll on purpose — the delta this tick just
  // consumed is the row's last chance to prove it still exists (any upsert
  // clears the stamp) before promotion. Full-pass only: the targeted root_id
  // branch is a hand tool, and promotion is estate-wide, not per-root.
  //
  // SYNC-1b (review D2): promotion runs ONLY on a tick that can vouch the
  // whole estate's delta was consumed. A stamped row's heal is a re-add in
  // that delta; if a root errored, was claimed by another instance, stopped
  // on a budget/time cap, or was never reached at all, its re-adds may sit
  // in pages this tick never fetched — promoting then would recreate the
  // exact invisible-documents window this arc exists to close, just rarer
  // (a Dropbox outage or a >30-minute reorganization backlog). Deferring is
  // free: the stamp keeps, the grace clock is already expired, and the first
  // fully-complete tick promotes everything at once. `empty_root` results
  // pass the gate: they carry stop:'complete' because a root with no folder
  // is a trivially completed walk (SYNC-1c/B1 — without that key, two
  // permanently-empty live roots held the gate shut on every tick and
  // promotion was dead on arrival).
  const unreached = result.roots_considered - result.roots.length;
  const unvouched = result.roots.find(
    (r) => r.error || r.skipped || r.stop !== 'complete');
  if (result.roots_considered === 0) {
    // N2: zero enabled roots is a maintenance or misconfiguration state —
    // an operator disabled the estate, or every root row is gone. Promotion
    // is the irreversible half of the tombstone, and an irreversible action
    // must not fire off an estate that is not syncing. (Note the vacuous
    // logic cuts BOTH ways here — "no delta could heal a stamp" argues for
    // promoting as much as against — which is exactly why the tiebreak is
    // the operational state, not the truth table.)
    output.pending_promotion_deferred = 'no enabled roots — no delta to vouch';
  } else if (unreached > 0) {
    output.pending_promotion_deferred = `${unreached} root(s) never reached (budget/deadline)`;
  } else if (unvouched) {
    output.pending_promotion_deferred =
      unvouched.error   ? `root ${unvouched.root_id} errored`
      : unvouched.skipped ? `root ${unvouched.root_id} claimed elsewhere`
      // `|| 'an unknown state'`: any FUTURE early-return shape that forgets a
      // `stop` key defers (correct, fail-closed) but must say something
      // diagnosable rather than "stopped on undefined".
      : `root ${unvouched.root_id} stopped on ${unvouched.stop || 'an unknown state'}`;
  } else {
    const documents = require('../../services/documentService'); // lazy require (convention)
    output.pending_promoted = await documents.promoteExpiredPendingDeletes(db);
  }
  ...
  console.log(
    ...
    `pending_promoted=${output.pending_promotion_deferred != null
      ? `deferred (${output.pending_promotion_deferred})`
      : output.pending_promoted}`
  );
...
fns.documents_sync.__meta = {
  ...
  description:
    ...
    'after every page, so the ~150k-file backfill completes across successive runs. Delta ' +
    'deletes stamp pending_delete_at rather than flipping status (SYNC-1); on a tick whose ' +
    'walk fully completed for every root, stamps older than 30 minutes are promoted to ' +
    "status='deleted' (output.pending_promoted, or pending_promotion_deferred with the reason). " +
    'Fail-closed on app_settings documents_sync_enabled != "1".',
  params: [
