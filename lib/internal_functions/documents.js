...
fns.documents_sync = async (params = {}, db) => {
  ...
  if (result.roots_considered === 0) {
    // N2: with no enabled roots there is no delta at all — nothing can heal
    // a stamp, so nothing may promote one. (Vacuous-truth promotion here
    // would have been "safe" only by accident of the estate never being in
    // this state.)
