...
// as of 2026-08-17 — see ref/archive/ORIGIN_SEPARATION_ROLLOUT.md. This file
// mints no staff-shell links, so it needs no app_url form.
...
fns.request_decision = async (params, db) => {
  ...
  // ── Context HTML (2026-09, website-lead routing) ─────────────────────
  // Optional block rendered on the /d/:token pages under the question, so
  // the responder sees WHAT they are deciding about without leaving the
  // page. TRUSTED HTML from workflow config — exactly the email_html trust
  // contract: template authors must pre-escape any untrusted values they
  // interpolate (the lead formatter esc()'s every user-supplied field).
  // Stored on decision_requests.context_html; null when absent.
  let contextHtml = params.context_html == null ? null : String(params.context_html);
  if (contextHtml != null && contextHtml.trim() === '') contextHtml = null;
  if (contextHtml != null && contextHtml.length > 200000) {
    throw new Error('request_decision: context_html exceeds 200000 chars');
  }
  ...
  // ── Delivery override (2026-09, website-lead routing) ────────────────
  // recipient_email / recipient_phone may OVERRIDE the resolved user/contact
  // delivery values without changing kind semantics: the decision stays bound
  // to the user/contact (paired task, decision_requests row, outcome log all
  // unchanged) — only where the message lands moves. Concretely: Stuart's
  // users.email is stuart@4lsg.com but he reads ssandweiss@gmail.com; flipping
  // users.email would ripple into every other email the system sends him.
  // Blank/absent = no override (every pre-existing config behaves exactly as
  // before; raw is untouched — its values were read above).
  if (kind !== 'raw') {
    const emailOverride = String(params.recipient_email ?? '').trim();
    const phoneOverride = String(params.recipient_phone ?? '').trim();
    if (emailOverride) email = emailOverride;
  ...
  const [ins] = await db.query(
    `INSERT INTO decision_requests
       (token, workflow_execution_id, step_number, resume_step, recipient_kind,
        ...
        question, context_html, options, result_var, timeout_value, expires_at, status)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending')`,
    [
      token, executionId, Number.isInteger(stepNumber) ? stepNumber : 0, nextStep, kind,
      recipientUserId, recipientContactId, emailTo, smsTo,
      question, contextHtml, JSON.stringify(options), resultVar, timeoutValue, expiresAtIso,
    ]
  );
...
fns.request_decision.__meta = {
  ...
  params: [
    ...
    { name: 'recipient_email', type: 'string', required: false, placeholderAllowed: true,
      description: 'recipient_kind=raw: the destination address. kind=user/contact: optional OVERRIDE of the resolved email — delivery only, the paired task and decision binding stay on the user/contact.' },
    { name: 'recipient_phone', type: 'string', required: false, placeholderAllowed: true,
      description: 'recipient_kind=raw: the destination phone. kind=user/contact: optional OVERRIDE of the resolved phone — delivery only.' },
    ...
    { name: 'context_html', type: 'string', required: false, placeholderAllowed: true, multiline: true,
      description: 'Optional HTML block shown on the /d/ decision pages under the question (and on the confirm/answered views), so the responder sees what they are deciding about. TRUSTED HTML — same contract as email_html: pre-escape any untrusted values. Max 200k chars.' },
