...
fns.enroll_sequence.__meta = {
  ...
  params: [
    ...
    { name: 'appt_with', type: 'integer', required: false,
      modeGroup: 'by_type', placeholderAllowed: true,
      description: 'Cascade filter (type-mode only). Accepts a {{placeholder}} — this function\'s own JSDoc example has always shown "appt_with": "{{apptWith}}".' },
    { name: 'template_id', type: 'integer', required: false,
      modeGroup: 'by_id', placeholderAllowed: true,
      description: 'Specific template ID for direct enrollment. Accepts a {{placeholder}} — the runtime parseInt()s it and rejects a non-positive result.', example: 42 },
