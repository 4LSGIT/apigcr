...
fns.start_workflow = async (params, db) => {
    ...
    const { advanceWorkflow, resolveExecutionContactId, captureWorkflowInput } = require('../workflow_engine');
    ...
    if (!wfRow.current_version) {
      throw new Error(`start_workflow: workflow #${workflowIdNum} has never been published — publish it before targeting it`);
    }
    if (!wfRow.active) {
