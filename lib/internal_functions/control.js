...
// CONTROL FLOW
//
// ENGINE COUPLING: `controlFlow: true` in a function's __meta is what makes
// workflow_engine.js isControlStep() honor its next_step — the flag IS the
// registration (it was a hardcoded whitelist in the engine until 2026-08; the
// two drifted, silently). Any function returning next_step WITHOUT
// delayed_until must carry the flag or the engine sequential-advances past it.
// Functions that only ever return next_step alongside delayed_until don't need
// it (the delay path reads next_step directly) — but flag them anyway if a skip
// path is plausible. Also add the function to BRANCH_TARGET_PARAMS in
// routes/workflows.js so its step targets survive renumbering; both invariants
// are asserted by tests/control.flow.test.js.
// ─────────────────────────────────────────────────────────────

/**
 * set_next
 * Jump to a specific step number, or terminate with 'end' / 'cancel' / 'fail'.
 *
 * params:
 *   value  {number|'end'|'cancel'|'fail'|null}
 *          — target step; 'end' (or null, or blank) ends the workflow normally
 *
 * Sentinel resolution lives in ONE place: workflow_engine.normalizeNextStep().
 * This function is a pass-through — it does not interpret its own value.
 *
 * 'end' is the word form of null, added 2026-08 so that a COMPUTED target can
 * express "end": set_next { value: "{{jump_to}}" } could previously resolve to
 * a step number, "cancel" or "fail", but had no way to say "stop normally".
 *
 * example config:
...
fns.set_next.__meta = {
  ...
  workflowOnly: true,
  controlFlow: true,
  description: 'Jump to a specific step number, or terminate with end/cancel/fail.',
  params: [
    ...
    //
    // "end" is the preferred authoring form; explicit null and a blank field
    // remain equivalent (nullTerminates keeps the blank-field path working).
    // The engine also still accepts the string "null" as a DEPRECATED alias —
    // wf41 s5/s7 shipped with it — but it is not advertised here.
    { name: 'value', type: 'string', required: true, placeholderAllowed: true,
      nullishSkipsBlock: true, nullTerminates: true,
      description: 'Step number, "end" (end normally), "cancel", or "fail". Blank/null also ends normally.',
      example: 5 },
  ],
  example: { value: 5 }
};
 ...
 *   MAX_STEPS_PER_INVOCATION=20 schedules a self-continue rather than
 *   failing, and the engine's runaway-loop guard (workflow_engine.js,
 *   2026-09-22) does not count a loop-back that lands here while this cursor
 *   is ADVANCING (only a 1,000-pass budget between pauses applies) — so
 *   foreach must bound itself. (A loop-back whose cursor did not move — this
 *   step threw and 'ignore' fell into the body — is counted.) It therefore
 *   (a) bounds the list at max_items
...
fns.foreach = async (params) => {
    ...
    // end_step: same value contract as set_next.value (resolved by
    // workflow_engine.normalizeNextStep — step number / "end" / "cancel" /
    // "fail" / null). undefined is a config error (nullishSkipsBlock makes
    // explicit-null the deliberate form).
...
fns.foreach.__meta = {
  ...
  params: [
    ...
    { name: 'end_step', type: 'string', required: true, placeholderAllowed: true,
      nullishSkipsBlock: true, nullTerminates: true,
      description: 'Where to go when the list is exhausted: step number, "end" (end the workflow), "cancel", or "fail". Blank/null also ends the workflow.', example: 12 },
