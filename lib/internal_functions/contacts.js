...
fns.lookup_contact = async (params, db) => {
    ...
    // S5-A: the Clio id moved to the custom field `cf_clio_id`; this list
    // reads the LIVE value, not the frozen `contact_clio_id` column (which
    // Phase B drops). `cf_clio_id` is a VIRTUAL column the reconciler owns,
    // so it can be absent — before the def is created, or after an admin
    // deactivates it. That must not take every lookup_contact down with it:
    // on the exact ER_BAD_FIELD_ERROR, retry once without the column and
    // warn. Any other error propagates untouched.
    const CORE_COLS = 'contact_id, contact_kind, contact_org_name, contact_fname, contact_lname, contact_name, contact_pname, contact_phone, contact_phone2, contact_email, contact_email2, contact_type, contact_address, contact_city, contact_state, contact_zip, contact_dob, contact_marital_status, contact_ssn, contact_tags, contact_notes, contact_created';
    let row;
    try {
      [[row]] = await db.query(
        `SELECT ${CORE_COLS}, cf_clio_id FROM contacts WHERE contact_id = ?`,
        [contact_id]
      );
    } catch (err) {
      if (!err || err.code !== 'ER_BAD_FIELD_ERROR') throw err;
      console.warn('[LOOKUP_CONTACT] cf_clio_id column absent (def missing or deactivated) — returning core columns only');
      [[row]] = await db.query(
        `SELECT ${CORE_COLS} FROM contacts WHERE contact_id = ?`,
        [contact_id]
      );
    }
...
fns.lookup_contact.__meta = {
  ...
  description:
    ...
    'The Clio id is {{this.cf_clio_id}} (S5: it moved out of contact_clio_id into a custom ' +
    'field); it is absent from the output if that field is ever deactivated.',
  params: [
 ...
 * own whitelist of CORE columns: caseService.updateCase rejects unknown keys
 * since custom-fields S2 but still accepts every real column, so there the
 * fn-side list is the narrower gate.)
 *
 * Custom fields (S2): cf_ keys pass through to the service, which validates
 * each against the ACTIVE contact defs in the field registry — nothing to add
 * here, and no list to keep.
 *
