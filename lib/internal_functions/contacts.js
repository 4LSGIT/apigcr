 ...
 * so the SQL lives in the service now and this function only decides what the
 * automation surface is ALLOWED to hand it:
 *
 *   - contact_ssn is stripped, silently. It is on the service whitelist
 *     because the staff routes legitimately write it; the automation surface
 *     may not. Stripping rather than throwing keeps a workflow from dying on a
 *     field its author had no business setting. (It also keeps SSN out of the
 *     step log below, which used to print whatever `fields` contained.)
...
fns.update_contact = async (params, db) => {
    ...
    // Scalar-only surface — see the header. Checked before the ssn strip so an
    // aggregate array is always the reported error, never "no writable fields".
    ...
    // Copy, don't mutate: `params` belongs to the engine, which has already
    // logged it, and the service does not mutate what it is handed either.
    const patch = { ...fields };
    delete patch.contact_ssn;
    if (Object.keys(patch).length === 0) {
      throw new Error('update_contact: no writable fields');
    }

    console.log(`[UPDATE_CONTACT] id=${contact_id} fields=${JSON.stringify(patch)}`);
    ...
    const result = await contactService.updateContact(db, contact_id, patch, {
      userId: 0,
      force: false,
    });
...
fns.update_contact.__meta = {
  ...
  description:
    ...
    'Scalar columns only: phones / emails / addresses arrays are rejected here. contact_ssn is ' +
    'silently dropped — the automation surface may not write it.',
  params: [
    { name: 'contact_id', type: 'string', required: true, placeholderAllowed: true,
      example: '{{contactId}}' },
