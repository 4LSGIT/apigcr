...
fns.lookup_contact.__meta = {
  ...
  description:
    ...
    'using contact_fname, which is blank on org contacts.',
  params: [
 ...
 *   contact_name     — trigger-computed from fname/mname/lname (or org_name)
 ...
 *
 * KIND (org-contacts slice 1). contact_kind ('person'|'org') and
 * contact_org_name are writable here, but THIS FUNCTION DOES NOT RUN THE
 * KIND-SWITCH GUARD that contactService.updateContact runs — it writes
 * straight to the table. Flipping contact_kind to 'org' from a workflow
 * without also setting contact_org_name (or vice versa) leaves the derived
 * names blank, silently: the columns are NOT NULL and '' is legal under this
 * session's non-strict sql_mode. Always set BOTH in the same call. The
 * shape check below is a minimum, not a substitute for the service guard.
 *
 * params:
...
fns.update_contact = async (params, db) => {
    ...
    // Minimum kind hygiene. The triggers branch on an EXACT 'org' match, so
    // 'Org' or 'organization' would land the row in the person branch and
    // compute names from blank parts. Cheap to reject; expensive to find later.
    if ('contact_kind' in fields) {
      const k = String(fields.contact_kind || '').trim().toLowerCase();
      if (k !== 'person' && k !== 'org') {
        throw new Error(`update_contact: contact_kind must be "person" or "org" (got "${fields.contact_kind}")`);
      }
      fields.contact_kind = k;
      if (k === 'org' && !('contact_org_name' in fields)) {
        throw new Error('update_contact: switching contact_kind to "org" requires contact_org_name in the same call');
      }
    }
