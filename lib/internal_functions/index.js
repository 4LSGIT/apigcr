...
// ─────────────────────────────────────────────────────────────
// BARE-SCALAR MAPPING STRINGS
//
// A params_mapping string is a dot-path unless single-quoted, so "22", "0",
// "true", "null" are lookups of keys literally NAMED 22 / 0 / true / null on
// the transform output — undefined in every real config. Nobody authors that
// on purpose: it is what a stored JSON number became after a lossy UI round
// trip (public/automation/paramsMapping.js before _parseValue). Live
// 2026-09-22: trigger rule 19's create_task had been saved with
// assigned_to 22 / assigned_by 0; a UI re-save to add two email actions
// rewrote them as "22" / "0", and every later fire failed with
// "create_task requires assigned_to" while the rule's emails still went out.
//
// Rejected at save time on every surface that calls validateParamsMapping
// (trigger actions, email-ingest actions, hook targets). Leading-zero digit
// strings ("007") are rejected too — equally meaningless as a path; quote
// them if the literal is intended.
// ─────────────────────────────────────────────────────────────
const PM_BARE_SCALAR_RE = /^\s*(?:-?\d+(?:\.\d+)?|true|false|null)\s*$/;
 ...
 *
 * Only checks what is knowable before dispatch: (1) any mapping string that is
 * a bare JSON scalar ("22", "true" — see BARE-SCALAR MAPPING STRINGS), and
 * (2) params declared csvList whose mapping value is in literal form.
 * Everything else passes through to the runtime guard. Returns null on success, or { status, error, param } on
 * failure — { status, error } matches validateInternalFunctionParams so callers
 * handle both alike; `param` is the offending param NAME, for callers that
 * report errors as {field, message} pairs (the ingest validators) rather than
 * as a thrown 400 (triggerService).
 */
function validateParamsMapping(functionName, paramsMapping) {
  ...
  // Bare-scalar STRINGS — generic across every param and every function,
  // meta-less ones included, so it runs before the meta gate below.
  for (const [param, source] of Object.entries(paramsMapping)) {
    if (typeof source === 'string' && PM_BARE_SCALAR_RE.test(source)) {
      const v = source.trim();
      return {
        status: 400,
        param,
        error: `${functionName} params_mapping.${param}: "${v}" is a dot-path lookup of a `
             + `key named ${v} (always undefined), not the value ${v}. Store it as a JSON `
             + `value (${v} unquoted in the JSON) or as a literal string ('${v}').`,
      };
    }
  }
  ...
  for (const spec of meta.params) {
    ...
    if (err) return { status: 400, param: spec.name, error: err.error };
  }
