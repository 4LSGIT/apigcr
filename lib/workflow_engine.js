...
async function executeStep(step, resolvedConfig, context, db) {
  ...
  try {
    ...
    console.log(`[EXECUTE STEP ${stepId}] Success`);
  } catch (err) {
    console.error(`[EXECUTE STEP ${stepId}] Failed: ${err.message}`);
    return { success: false, error: err.message, output: null, set_vars: {} };
  }

  // Update 'this' for the current step (so {{this}} works in set_vars)
  context.this = rawResult;

  // Resolve static set_vars from config **AFTER** execution (so {{this}} works)
  let staticSetVars = {};
