    ...
    while (executedThisInvocation < MAX_STEPS_PER_INVOCATION) {
      ...
      if (controlTarget) {
        if (controlTarget.kind === 'invalid') {
          ...
          await markExecutionCompleted(executionId, 'failed', db);
          return { status: 'failed', message: `Step ${currentStepNumber}: ${stepResult.error}` };
        }
        ...
        if (controlTarget.kind === 'cancel' || controlTarget.kind === 'fail') {
          const normalizedStatus = controlTarget.kind === 'cancel' ? 'cancelled' : 'failed';
      ...
      if (stepResult.delayed_until) {
        ...
        if (normResume.kind !== 'step') {
          ...
          await markExecutionCompleted(executionId, 'failed', db);
          return { status: 'failed', message: `Step ${currentStepNumber}: ${msg}` };
        }
        await scheduleResume(executionId, stepResult.delayed_until, normResume.step, db);
...
async function scheduleResume(execId, resumeAt, nextStep, db) {
  ...
  // skips the enqueue (cron-only). The dedupe early-return above enqueues
  // nothing — a pending resume already has its task.
  ...
  if (Number.isFinite(resumeAtMs) && resumeAtMs <= Date.now() + ACCEL_WINDOW_MS) {
    await enqueueJobDispatch(insertResult.insertId, resumeAtMs);
  }
}
