...
// NOTE: phoneIngestSuppressionService is lazy-required INSIDE phone_log,
// matching this file's circular-dep-safety convention for logService /
// sequenceEngine / apptService. Do NOT add a module-scope require for it.
 ...
 * FIRM_DOMAINS load: firm lines change ~never, and a deploy reloads. If a
 * line is added without a deploy, call _resetFirmNumberCache() or just
 * redeploy — staleness only affects the firmToFirm flag, never logging
 * correctness.
 */
...
function _resetFirmNumberCache() { _firmNumberCache = null; }
...
// ─────────────────────────────────────────────────────────────
  ...
  phone_log: async (params, db) => {
    const logService = require('../services/logService');
    const phoneIngestSuppressionService = require('../services/phoneIngestSuppressionService');
    ...
    if (suppressed) {
      ...
      return {
        success: true,
        output: { log_id: null, suppressed: true, matched_rule_ids: matchedRuleIds, firmToFirm },
      };
    }
    ...
    return { success: true, output: { ...result, suppressed: false, firmToFirm } };
  },
...
internalFunctions.__resetFirmNumberCache = _resetFirmNumberCache;
