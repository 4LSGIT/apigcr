...
// ─────────────────────────────────────────────────────────────
// HELPER: direction normalization  (Slice 4-C)
// ─────────────────────────────────────────────────────────────
//
// External providers use varied direction labels ("Inbound"/"Outbound"
// from RingCentral, "incoming"/"outgoing" from internal callers). The
// `log.log_direction` column is an ENUM('incoming','outgoing') and would
// silently reject mismatched values — which is the bug this normalization
// fixes. Normalization happens at the internal-function boundary so
// workflow variables retain their raw upstream value (useful for
// `evaluate_condition` checks like the wf 15 step 3 dedupe), while the
// DB write conforms to the enum.
//
// Unknown values pass through unchanged for forward compatibility — if
// a future provider supplies a third direction label, the enum will
// reject it loudly rather than this normalizer silently dropping it.
const DIRECTION_NORMALIZATION = {
  'inbound':  'incoming',
  'incoming': 'incoming',
  'outbound': 'outgoing',
  'outgoing': 'outgoing',
...
function normalizeDirection(d) {
  ...
  return DIRECTION_NORMALIZATION[lower] ?? d;
}
...
// ─────────────────────────────────────────────────────────────
  ...
  create_log: async (params, db) => {
    ...
    // Slice 4-C: normalize direction at the function boundary. The
    // workflow variable keeps the raw upstream value (e.g. "Outbound")
    // for branching checks; only the DB write conforms to the enum.
    ...
    console.log(`[CREATE_LOG] type=${type} link=${link_type}:${link_id} by=${by} direction=${direction}\u2192${normalizedDirection}`);
