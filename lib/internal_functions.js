...
const internalFunctions = {
   ...
   * a row update without code changes here. If the flag is false, this throws
   * a clear error — no silent fallback to SMS.
   *
   * MEDIA TYPES:
   ...
   *
   * CONTENT-TYPE GOTCHA:
   *   ringcentralService.sendMms strips Content-Type parameters before
   *   forwarding to RC because RC's parser doesn't normalize them. A source
   *   URL that returns `application/pdf; qs=0.001` (some W3C-hosted files do
   *   this as a content-negotiation hint) gets rejected as MSG-348
   *   "Unsupported attachment media type" if the parameter isn't stripped.
   *   Hosting attachments on GCS, your own server, or any provider that
   *   returns a clean Content-Type avoids the issue.
   *
   * params:
   ...
   *                                RingCentral fetches the file at send time
   *                                and caps it at 1.5MB.
   *
   * example config:
   *   {
  ...
  send_mms: async (params, db) => {
    // Lazy require — keeps ringcentralService scoped to this one function rather
    // than as a top-of-file import. Resolves to the module already loaded via
    // smsService → ringcentralService chain, so no real cost.
    ...
    if (!line) throw new Error(`send_mms: no phone line found for ${from}`);
    if (!line.active) throw new Error(`send_mms: phone line ${from} is inactive`);
    if (!line.mms_capable) {
      throw new Error(`send_mms: phone line ${from} is not MMS-capable (provider=${line.provider}) — use send_sms with a link instead`);
    }

    console.log(`[SEND_MMS] from=${fromClean} to=${to}`);
    const result = await ringcentralService.sendMms(
      db, fromClean, to,
      text || '',
      'US',
      null, null, null,    // no buffer / filename / mimetype — URL path
      attachment_url,
      false                // don't store attachment to GCS
    );
