...
// ─────────────────────────────────────────────────────────────
   ...
   *             a does_appts user id = blocks only that provider).
...
internalFunctions.create_event.__meta = {
  ...
  params: [
    ...
    { name: 'event_with', type: 'integer', required: false, placeholderAllowed: true,
      description: 'users.user (does_appts=1). Scopes which provider\'s booking availability a timed event blocks: omit/null = blocks ALL providers (firm-wide); an id = blocks only that provider.' },
    { name: 'acting_user_id', type: 'integer', required: false, default: 0,
