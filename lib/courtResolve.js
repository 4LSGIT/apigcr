...
async function resolveCase(db, docket) {
  ...
  let [rows] = await db.query(
    `SELECT case_id, case_number, case_number_full, case_chapter, case_caption
       FROM cases
  ...
  // Primary)… with one override: PERSONS BEFORE ORGS, regardless of relate
  // rank. Every consumer of primary_contact_id is comms-flavored — the 341
  // appt (and its createAppt sequence auto-enrollment), the court_processed
  // domain event, review-task text, and {{primary_contact_id}} substitutions
  // in AI-authored actions. An organization cannot answer a phone: when a
  // Ch11 debtor org is the case's Primary, the human officer (typically
  // Secondary) is who reminders and events must target. Documents are
  // unaffected — debtor identity on filings flows through case_relate
  // Primary directly, not through this resolver. All-person cases are
  // byte-identical to the old ordering (kind is a constant there).
  // Incident ref: sequence 19 enrollment 176 / appt 3980 (2026-09-09) —
  // org Primary enrolled in an SMS sequence with no phone.
  const [contacts] = await db.query(
    `SELECT co.contact_id, co.contact_name
      ...
      ORDER BY (co.contact_kind = 'person') DESC,
               FIELD(cr.case_relate_type, 'Primary','Secondary','Other','Bystander'),
               co.contact_name
      LIMIT 1`,
    [c.case_id]
  );
  ...
  return {
    ...
    case_caption: c.case_caption || null,
