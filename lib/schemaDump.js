...
// ─────────────────────────────────────────────────────────────────────────────
async function schemaFingerprint(db, opts = {}) {
  const onQuery = typeof opts.onQuery === "function" ? opts.onQuery : null;

  const t0 = Date.now();
  const [[{ db: dbName }]] = await db.query("SELECT DATABASE() AS db");
  if (onQuery) onQuery({ name: "database()", ms: Date.now() - t0, rows: 1 });

  // Sequential, not Promise.all. Counter-intuitive, but measured: on this setup
  // a query costs ~40 ms and a NEW CONNECTION costs ~4 s. Fanning 7 queries out
  // across a pool opens 7 connections and pays the handshake 7 times; running
  // them in series on one warm connection pays it once. It also stops a
  // superuser hitting /admin/db/schema.sql from grabbing 7 of the app pool's
  // 10 connections at once.
  const results = {};
  const q = async (name, sql, tolerateMissing = false) => {
    const started = Date.now();
    let rows;
    try {
      [rows] = await db.query(sql, [dbName]);
    } catch (err) {
      if (!tolerateMissing) throw err;
      rows = [];
    }
    if (onQuery) onQuery({ name, ms: Date.now() - started, rows: rows.length });
    results[name] = rows;
  };
  ...
  await q("checks",
    `SELECT CONSTRAINT_NAME, CHECK_CLAUSE
      ...
      ORDER BY CONSTRAINT_NAME`, true);          // absent before MySQL 8.0.16
  ...
  const payload = JSON.stringify(results, (_k, v) =>
...
async function buildSchemaDump(db, source, opts = {}) {
  ...
  //
  // SHOW CREATE TABLE can't be batched, so this is one round trip per table
  // (117 today). Run them N-at-a-time rather than serially — on a remote DB the
  // wall clock here is latency, not MySQL work. mapLimit preserves input order,
  // so the dump stays byte-identical regardless of completion order.
  const onProgress = typeof opts.onProgress === "function" ? opts.onProgress : null;
  let fetched = 0;
  const ddls = await mapLimit(tableRows, opts.concurrency || 6, async ({ name }) => {
    const [[row]] = await db.query(`SHOW CREATE TABLE \`${name}\``);
    if (onProgress) onProgress({ phase: "tables", done: ++fetched, total: tableRows.length });
    return { name, ddl: row["Create Table"] };
  });
