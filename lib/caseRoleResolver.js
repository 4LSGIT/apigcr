 ...
 * NOT reimplement its rules here) against the fe-trustees roster, then maps
 * roster entry → the entry's `contact_id` field, which
 * scripts/seedRoleContacts.js added to the setting. An entry with no
 * contact_id returns null — never guess. `case_chapter` is passed through to
...
/** Load + parse the fe-trustees roster. null on missing/unparseable. */
async function _loadRoster(db) {
  // Lazy require: settingsService is trivially cheap but keeping the lib's
  // top-level dependency-light follows the internal_functions convention.
