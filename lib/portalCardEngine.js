 ...
 * client portal case view AND (S5) the case-less home page. Staff define
 * cards (E2 admin UI); this module is the ONLY render path —
 * public/portal/case.html and public/portal/home.html consume its output,
 * and the E2 admin preview renders THROUGH this same path (previewCard →
 * renderOneCard), never a parallel renderer that can drift.
 *
 * S5 HOME SURFACE (2026-08-09) — placement 'home' renders on the portal
 * home page, where THERE IS NO CASE: renderCards runs with caseId null.
 * Home-placement cards may therefore reference the CONTACTS side of the
 * whitelist ONLY (HOME_ALLOWED_TABLES). Enforced at SAVE time
 * (validateCard is placement-aware: template refs, rules fields, AND sql
 * paramMap paths), with render-time fail-closed backstops for each of the
 * three (scanTemplateViolations placement arg / getField anchor gate /
 * evaluateSqlCondition case-less param gate). Case-placement behavior is
 * unchanged. Coded cards are case-view renderers — a coded_key on a home
 * card is a save-time rejection AND a render-time hide (new home renderers
 * would ship as code, together with a home.html renderer map).
 *
...
// 'case_top' vs 'case' (above/below the timeline); home.html (S5) renders
// placement 'home'. Save-time validation pins to this set: an unknown
// placement is a card that renders NOWHERE — the silent vanish E2 exists to
// prevent. Extending means adding here + a client layout, together.
const VALID_PLACEMENTS = ['case_top', 'case', 'home'];

// S5: the tables whose whitelist fields the HOME surface may reference.
// The home render context has NO case (caseId null) — cases.* refs there
// would evaluate against nothing, so they are rejected at save and fail
// closed at render. A CODE CONSTANT like the whitelist itself.
const HOME_ALLOWED_TABLES = new Set(['contacts']);
...
// R2 (2026-08-09) adds 'docsNav' — the Documents nav card, CODED because
// its link is per-case (docs.html?id=<case_id>): a static link_url cannot
// carry it and URL templating is deliberately NOT being added.
const KNOWN_CODED_KEYS = ['meeting341', 'docsNav'];
 ...
 * PORTAL_FIELD_WHITELIST (→ the rule fails closed).
 *
 * S5 anchor gate: when the field's table has NO anchor id in the session
 * context at all (home render: ctx.caseId is null), the entity class does
 * not exist on this surface — { ok:false }, rule fails CLOSED. Evaluating
 * against an implicit all-NULL row instead would make e.g. an `empty` rule
 * on cases.* PASS on the home page — exactly the leak the placement gate
 * exists to prevent. With an anchor PRESENT but no matching row (deleted
 * entity), the E1 semantics stand unchanged: value null — empty/date ops
 * behave as if the column were NULL. (Case-view renders always carry both
 * anchors, so case-placement behavior is unchanged.)
 */
function getField(ctx, field) {
  ...
  const anchor = table === 'cases' ? (ctx && ctx.caseId) : (ctx && ctx.contactId);
  if (anchor === null || anchor === undefined || anchor === '') {
    return { ok: false };            // no anchor in this context — fail closed
  }
...
function evaluateLeafRule(rule, ctx) {
  ...
  if (!f.ok) {
    // Non-whitelisted field OR (S5) a field whose table has no anchor in
    // this context (e.g. a cases.* rule on the case-less home surface).
    console.warn(`[portalCardEngine] rule field not available in this context: ${rule.field} — fail closed`);
    return false;
  }
...
// S5: the subset a HOME-placement card's paramMap may use — the home render
// context has no case, so case_id paths are rejected at save
// (validateSqlConditionShape) and refused at render (the case-less gate in
// evaluateSqlCondition, which is context- rather than placement-driven and
// therefore also backstops rows that bypassed validation).
const ALLOWED_SQL_PARAM_PATHS_HOME = new Set([
...
async function evaluateSqlCondition(db, cond, ctx) {
  ...
  for (const [name, path] of Object.entries(paramMap)) {
    ...
    // S5 case-less backstop (context-driven, not placement-driven — also
    // covers rows that bypassed save validation): a case_id path with no
    // case in the render context (home surface) refuses BEFORE any SQL —
    // never bind NULL where the config asked for the session's case.
    if ((path === 'case_id' || path === 'trigger_data.case_id') &&
        (ctx.caseId === null || ctx.caseId === undefined || ctx.caseId === '')) {
      console.error(
        `[portalCardEngine] sql condition param :${name} maps to '${path}' ` +
        `but this render context has no case — fail closed`
      );
      return false;
    }
  }
 ...
 *
 * S5: pass placement 'home' to ALSO violate whitelisted refs outside
 * HOME_ALLOWED_TABLES — the home surface has no case, so cases.* refs are
 * refused there even though the case view may use them. Omitted/other
 * placements keep the E1 whitelist-only behavior.
 */
function scanTemplateViolations(text, placement) {
  const scan = scanExpressionRefs(text);
  const violations = [];
  if (scan.triggerData) violations.push('trigger_data.*');
  const homeOnly = placement === 'home';
  for (const { table, column } of scan.refs) {
    const key = `${table}.${column}`;
    if (!PORTAL_FIELD_WHITELIST.has(key)) violations.push(key);
    else if (homeOnly && !HOME_ALLOWED_TABLES.has(table)) violations.push(key);
  }
  return violations;
}
...
async function renderOneCard(db, row, ctx, { fireAlerts = true } = {}) {
  ...
  if (row.body_type === 'coded') {
    ...
    if (row.placement === 'home') {
      // S5 backstop — save-time rejects coded home cards; a row that got in
      // anyway (direct SQL) is hidden here rather than shipped to a client
      // page that has no coded renderers.
      return { card: null, hidden_reason: 'coded cards are case-view renderers — hidden on the home surface' };
    }
    return {
      card: {
  ...
  const violations = scanTemplateViolations(tpl, row.placement);
 ...
 * @param {object} db
 * @param {object} o
 * @param {string|null} o.caseId  scope-confirmed case id (canonical casing).
 *                      S5: NULL on the case-less home surface — loadCtx then
 *                      skips the cases query entirely; case-field rules and
 *                      case_id sql params fail closed (getField anchor gate /
 *                      evaluateSqlCondition case-less gate).
...
function validateLeafRuleShape(rule, path, errors, placement) {
  if (typeof rule.field !== 'string' || !PORTAL_FIELD_WHITELIST.has(rule.field)) {
    errors.push(`${path}: field '${rule.field}' is not in the portal field whitelist`);
             ...
             !HOME_ALLOWED_TABLES.has(rule.field.slice(0, rule.field.indexOf('.')))) {
    // S5: whitelisted, but not a home-surface table — there is no case on
    // the home page for this rule to read.
    errors.push(
      `${path}: field '${rule.field}' is not available on the home surface — ` +
      `home cards may use contacts.* whitelist fields only (there is no case on the home page)`
    );
  }
...
function validateRuleGroupShape(group, path, depth, errors, placement) {
  ...
  group.rules.forEach((r, i) => {
    ...
    if (Array.isArray(r.rules)) validateRuleGroupShape(r, p, depth + 1, errors, placement);
    else validateLeafRuleShape(r, p, errors, placement);
  });
}

function validateSqlConditionShape(cond, errors, placement) {
  ...
  const allowedPaths = placement === 'home'
    ? ALLOWED_SQL_PARAM_PATHS_HOME : ALLOWED_SQL_PARAM_PATHS;
  ...
  if (!paramMap || typeof paramMap !== 'object' || Array.isArray(paramMap)) {
    ...
    for (const [name, path] of Object.entries(paramMap)) {
      if (typeof path !== 'string' || !allowedPaths.has(path)) {
        errors.push(placement === 'home'
          ? `sql condition param :${name} maps to '${path}' — home cards may only use the ` +
            `contact_id session path (optionally trigger_data.-prefixed); there is no ` +
            `case_id on the home page`
...
function validateCard(input) {
  ...
  // placement — resolved BEFORE the body/conditions checks (S5): the home
  // surface constrains which fields a template, rule, or sql paramMap may
  // reference, so those validators need it.
  ...
  if (body_type === 'coded') {
    if (placement === 'home') {
      // S5: coded renderers live in case.html — the home page has none.
      // New home renderers would ship as code (a home renderer map +
      // KNOWN_CODED_KEYS split), together — until then this is a save-time
      // rejection (renderOneCard hides such a row as the backstop).
      errors.push(
        `coded cards are case-view renderers — placement 'home' supports template cards only ` +
        `(home coded renderers would ship with code)`
      );
    }
    ...
    coded_key = null;
    const violations = scanTemplateViolations(
      body_template === null ? '' : body_template, placement);
    if (violations.length) {
      errors.push(placement === 'home'
        ? `body_template references fields not available on the home surface: ` +
          `${violations.join(', ')} — home cards may use contacts.* whitelist fields only ` +
          `(there is no case on the home page); cards refuse (never strip)`
    ...
    if (parsed.error) {
      ...
      if (parsed.mode === 'rules') {
        validateRuleGroupShape(
          { match: conditions.match, rules: conditions.rules }, 'conditions', 0, errors, placement
        );
      } else {
        validateSqlConditionShape(conditions, errors, placement);
      }
    }
  }
