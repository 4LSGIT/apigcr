...
function targetMatchesAppUrl(targetUrl) {
  // app_url setting → APP_URL env (deferred require — circular-dep safety).
  const appUrl = require('./firmConfig').cfg('app_url');
...
function checkUrlScope(credential, url) {
  ...
  if (credential.type === 'internal') {
    ...
    const appUrl = require('./firmConfig').cfg('app_url');
    if (!appUrl) {
      return { ok: false, reason: `app_url setting / APP_URL env not set, cannot verify ${url} is self-targeted` };
    }
    return targetMatchesAppUrl(url)
      ? { ok: true }
      : { ok: false, reason: `${url} does not match app_url (${appUrl})` };
  }
...
function buildAuthHeaders(credential, url) {
  ...
  if (credential.type === 'internal') {
    // Same source as the verifier (auth.jwtOrApiKey): internal_api_key
    // setting → INTERNAL_API_KEY env. Rotations propagate to both sides.
    return { 'x-api-key': require('./firmConfig').cfg('internal_api_key') };
  }
