...
router.post('/api/email-ingest/rules/:id/duplicate', jwtOrApiKey, async (req, res) => {
  try {
    const rule = await ruleService.duplicateRule(req.db, req.params.id, req.auth.userId);
    if (!rule) return res.status(404).json({ status: 'error', message: 'Rule not found' });
    auditEI(req, { details: { entity: 'rule', entity_id: rule.id, duplicated_from: Number(req.params.id), after: rule } });
    res.status(201).json({ status: 'success', rule });
  } catch (err) {
    console.error('[email-ingest] duplicate rule error:', err);
    res.status(500).json({ status: 'error', message: err.message });
  }
});
...
//
// POST /api/email-ingest/rules/test-transform
//
...
router.post('/api/email-ingest/rules/test-transform', jwtOrApiKey, async (req, res) => {
  try {
    const body = req.body || {};
    ...
    let envelope, source;
    if (execId !== undefined) {
      const corpus = await sampleService.fetchSyntheticEnvelopes(req.db, { exec_id: execId });
      if (corpus.not_found) {
        return res.status(404).json({
          error: corpus.not_found === 'no_email_log'
            ? `execution #${execId} has no email_log row — nothing to reconstruct (auth/validation failures never produced one)`
    ...
    console.error('[email-ingest] test-transform error:', err);
    res.status(500).json({ status: 'error', message: err.message });
  }
});
