...
const ALERT_URL = "https://connect.pabbly.com/workflow/sendwebhookdata/IjU3NjYwNTZhMDYzMTA0M2Q1MjY5NTUzNjUxMzUi_pc";
...
async function setSetting(db, key, value) {
    return new Promise((resolve, reject) => {
        db.query("REPLACE INTO app_settings (`key`, `value`) VALUES (?, ?)", [key, value], (err) => (err ? reject(err) : resolve()));
    });
}
...
async function refreshAccessToken(db) {
    ...
    refreshPromise = (async () => {
        try {
            ...
            tokenData = { ...(await res.json()), issued_at: Date.now() };
            await saveToken(db);
            scheduleRefresh(db);
            ...
            if (raw) {
                ...
                if (now >= tokenExpiry) {
                    ...
                    try {
                        ...
                        sendAlert("token_refresh_failed", "Token refresh failed on load", { error: err.message });
                        tokenData = null;
                    }
                } else {
                    scheduleRefresh(db);
...
async function checkApiKey(req, res, next) {
    ...
    if (key !== cachedApiKey) {
        sendAlert("unauthorized_access", "Invalid API key attempt", { ip: req.ip, path: req.originalUrl, provided_key: key || null });
...
router.get("/ringcentral/callback", async (req, res) => {
    ...
    try {
        ...
        tokenData = { ...(await resToken.json()), issued_at: Date.now() };
        await saveToken(db);
        scheduleRefresh(db);
        res.send("Authorization successful. You can now send SMS or fax.");
    } catch (err) {
...
router.all("/ringcentral/send-sms", checkApiKey, async (req, res) => {
    ...
    try {
        const smsRes = await fetchWithRetries("https://platform.ringcentral.com/restapi/v1.0/account/~/extension/~/sms", {
            method: "POST",
            headers: {
...
router.post("/ringcentral/send-fax", checkApiKey, async (req, res) => {
    const form = new formidable.IncomingForm();
    
    form.parse(req, async (err, fields, files) => {
        if (err) {
            console.error("Form parsing error:", err);
            sendAlert("fax_form_parse_failed", "Failed to parse fax form data", { error: err.message });
        ...
        if (!from || !to || !file) {
            return res.status(400).json({ error: "Missing required fields: from, to, or file" });
        }
        ...
        if (now >= tokenExpiry) {
            console.log("Token expired during fax send, refreshing...");
            try {
                await refreshAccessToken(db);
            } catch (err) {
                sendAlert("token_refresh_failed", "Failed to refresh access token during fax send", { from, to, error: err.message });
                return res.status(401).json({ error: "Failed to refresh access token" });
            }
        }
        ...
        try {
            ...
            formData.append('to', JSON.stringify([{ phoneNumber: to }]));
            formData.append('attachment', require('fs').createReadStream(file.filepath), {
                filename: file.originalFilename,
                contentType: 'application/pdf'
            });

            const faxRes = await fetchWithRetries(
            ...
            const result = await faxRes.json();
            res.json(result);
        } catch (err) {
            console.error("Fax send error:", err);
            sendAlert("fax_send_failed", "Fax send failed after retries", { from, to, error: err.message });
            res.status(500).json({ error: "Error sending fax" });
        }
    });
});
