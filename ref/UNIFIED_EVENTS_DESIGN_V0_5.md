...
**Status: RATIFIED 2026-09-01; U1–U6b shipped the same day (§7). `unified_singleton_enabled` still OFF.** Every gate in the
...
**U6c (Fred, 2026-09-03) overrules the E0a 'pointer, never a status' rule:** `events.event_status` gains `'Rescheduled'` (a third value — `'Canceled'` still means court-cancelled only). Status answers "is this live"; the pointer stays authoritative for identity and lineage; `supersede_reason` still separates `'rescheduled'` from `'duplicate'` (the 31 E0a rows keep `Canceled`). Rationale: appts always had both; events-only-pointer forced every liveness query to learn a novel predicate, and courtExecutor's two candidate queries never did — under the flag the court robot would have treated a dead predecessor as a live candidate. With the status set, availability, the digest, the feed, both dedupe scans and both executor sites are correct unedited.
...
**U1** — `ref/migrations/2026-09-01_unified_events_e0b.sql`: `events.kind` by type plus the four hand-fixes;
applied and verified 2026-09-01. **U2** — `ref/migrations/2026-09-01_unified_events_u2.sql` + backend +
...
| U2b | **As built (options model, Fred's mid-slice rulings):** `calendar_type_options` table (type_key, label override, **length**, surfaces `new_client`/`follow_up`, sort, active; UNIQUE `(type_key,length)`; no `option_key` — identity stays on the type) instead of a surfaces column; 13 seeded options reproduce both dialog lists (one deliberate move: Schedules 20 beside 45); pickers registry-first with the pre-U2b lists as byte-for-byte fetch-failure fallback, pass `case_type` (BK-scoped types hide on non-BK cases; re-fetch on case change); Case Config shell **already existed** (`caseConfigManager.html` + `caseconfig/`) — a tab was added, next slot reserved for U9; admin split route+`calendarTypeAdminService`; options get their own cache; `matrix_meeting` 30→15; no `staff_event` surface (kind scopes eventform); `kind` locks while options exist | Opus | U6b | **deployed + verified 2026-09-02** (192 / 6,204). contact.html call site verified: rows carry `case_type`; 0 compound old-shape values live |
| U8 | As built: `approaching_offsets` (NULL-seeded), claim PK `(source,source_id,offset_days,item_date)`, emitter with its own two-column registry query (committed truth, not the 60s cache) + copied `RESOLVED_CASE_SUBQUERY`, half-open datetime bounds on `appts.appt_date` (index-preserving), approaching guard = four-shape set **plus exactly** `offset_days`/`days_until`, dry-run reads the ledger (`duplicates` vs `would_emit`), `court_item_reminders` dropped. **Item 0 `cfg()` guard proved negatively** (names both a2745f72 readers with line numbers). Rulings: catch-up fires passed rungs (filter `days_until == offset_days` for on-the-nose), `source:'system'`, offsets normalized | Opus | U4, U6a, U2b | **deployed + verified 2026-09-02** (196 / 6,276 / 1; claims 0). Fred: rules → offsets (use `dischargeability_due` for the dry-run check — `schedules_deadline` has zero forward rows) → dry run → job |
| U9 | As built: `listRange` + `routes/api.calendarRange.js` (docket join walked backwards once, first-writer-wins on shared dockets — documented so nobody "fixes" it into the log-feed shape); label column shows `title` (no new default key); `'live'` asserted as a SQL complement, not `='Scheduled'` (the six blank-enum 2024 appts stay live); parity gate proves mapper reuse field-by-field; eventform grew its own deadline dialogs (met/moot; 400 retried, 500/404 not); `listUnmapped` narrowed to non-blank labels with `no_label` counts alongside (CAL accepted — the 8 untyped-2024 appts are a separate class, not a mintable worklist); shell identifiers `calx`-prefixed; fence test pins both old tabs' existence so U9b's deletion is deliberate | Opus | U2b, U6a/b, U8 | **deployed 2026-09-02** (199 / 6,404 / 1) |
| U6c | `events.event_status` += `'Rescheduled'`; `supersedeEvent` sets it alongside the pointer; §3.4 rule overruled (see §3.4). Consumers correct unedited; flag flip unblocked and **flipped 2026-09-03** (verified: enum live, flag `'1'`, 0 half-writes, 31 legacy rows untouched) | — | U6a/b | **deployed 2026-09-03** (ruled + built in a parallel CAL session; recorded here) |
| U10c | **U10-census** (read-only, no code): the §4 writer census — forms definitions (`definition` + `draft_definition`), court `applied_fields` reality over 90d, workflow capability vs use, code writers, per-column data reality, 341 mirror divergence — producing the disposition table and the plain-language page one that leads the SS worksheet | **Opus** | — | prompt issued 2026-09-03 |
| U7 | Court-v2 s2 on U6a+U6b+U6c: resolve/reconcile executor (the pointer-filter item is closed by U6c); 341 → appts; retire event `341`; dedupe the 12 **by singleton identity, not date**; CHECK constraint with strict-mode proof; `court_v2 schema_gaps` (`block_minutes` → `default_length`, `blocks_whole_day` → `blocks_default`) | **Fable** | U6 + SS worksheet | owner of U6 owns U7 (both managers) |
...
- **[Superseded by U6c]** — the three-way predicate divergence shrinks to belt-and-braces: with
  `event_status='Rescheduled'` set by the writer, status filters do the work everywhere and the
  pointer predicates are redundancy for hand-edited rows. U9b's "predicate unification" is mostly
  done. Original note kept below for history.
...
- **[Superseded by U6c]** Forward supersession no longer needs per-consumer pointer filters; the
  status carries it. The courtExecutor filter item for U7 is **closed**. Original note kept below.
...
- **Cross-arc `firmConfig` hazard (U6b finding).** Commit `a2745f72` (another arc, "g3 g4") edited
  `firmConfig.js` from a pre-U6a copy and silently dropped the `unified_singleton_enabled`
  registration; `cfg()` throws on unregistered keys and `createEvent` calls it outside the
  try/catch — every event create would have 500'd post-commit. **No live impact** (verified: zero
  createEvent attempts in the exposure window — max event_id unchanged; the failure mode writes the
  row before throwing, and none was written; no 5xx alerts). Restored in the U6b patch. The class
  fix is a coverage test — every `cfg('…')` literal in `services/`/`lib/`/`routes/` must exist in
  the registry — folded into U7 item 0. Coord rule: any arc touching `firmConfig.js` rebases on
  fresh HEAD first.
