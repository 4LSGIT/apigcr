...
| U8 | `calendar.approaching`: `approaching_offsets JSON` on the registry (seeded NULL — zero emissions at deploy), claim table with `item_date` in the PK (date moves self-invalidate), `emit_calendar_approaching` internal fn (stage_aged pattern), catalog entry + drift guard, Case Config offsets field, `court_item_reminders` dropped (A6 supersedes v0.4 E2). **Item 0: `cfg()` coverage guard test** (the a2745f72 class fix, moved up from U7 since U7 is parked) | **Opus** | U4, U6a, U2b | prompt issued 2026-09-02. Fred creates the nightly job + rules as data, offsets first |
