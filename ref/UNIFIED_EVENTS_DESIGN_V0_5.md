...
| U9 | `listRange` on the read layer (frozen shape + additive `display` labels; superseded hide-all by default, deliberately unlike listEvents option-B) + shell **Calendar** tab (kind chips, state, provider, range; old tabs untouched) + eventform resolution surfacing (deadline complete asks met/moot, 400-fallback) + mint-from-unmapped with adopt (the slice's only events/appts write, rule-9 honoured). **U9b later:** old-tab removal, `fe-event_types` retirement, predicate unification | **Opus** | U2b, U6a/b, U8 | prompt issued 2026-09-02 |
