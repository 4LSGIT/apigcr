...
> **Scope:** Workflow Engine, Sequence Engine, Scheduled Jobs, YisraHook.
...
Four subsystems share the `scheduled_jobs` queue and `/process-jobs` polling heartbeat:
...
### 2.1 Internal Functions (22 total)

Available in `lib/internal_functions.js`, callable by all three engines. Signature: `async (params, db) => { success, output? }`.
...
**Refs auto-built in sequences** from `enrollment.contact_id` + `trigger_data.{appt_id, case_id, task_id}`.
...
{ "function_name": "schedule_resume",  "params": { "resumeAt": "{{resume_24h}}", "nextStep": 4 } }
{ "function_name": "wait_until_time",  "params": { "time": "09:00", "timezone": "America/Detroit", "nextStep": 6 } }
```

Duration formats: `"30s"`, `"5m"`, `"2h"`, `"1d"`, ISO datetime, or ms number.
...
```
         ...
         target_type= http  workflow  sequence  internal_function
          ...
          │  services   │ │ Engine  │ │ Engine  │ │     (22 total)   │
          │ or internal │ │         │ │         │ │                  │
          │   routes    │ │         │ │         │ │                  │
          └─────────────┘ └────┬────┘ └────┬────┘ └────────┬─────────┘
                               │           │                │
...
**Shared helper.** All three INSERT sites (§5.21) go through `resolveExecutionContactId()` in `lib/workflow_engine.js`:
...
```sql
UPDATE hooks
   SET capture_mode = 0, captured_payload = ?, captured_at = NOW()
 WHERE id = ? AND capture_mode = 1
```
...
Update `manual/03-workflow-manager/` (or wherever) with:
...
### 5.21 `workflow_executions` Has Three INSERT Sites

A new execution row is created in three places:
...
**Consequence.** Adding a column to `workflow_executions`, or any new rule at execution-row creation time, means touching all three. Contact-tying was partly consolidated via `resolveExecutionContactId()` in `lib/workflow_engine.js` (§3.12) — a future slice could lift the whole INSERT into a shared `createWorkflowExecution()` helper. Until then, grep for `INSERT INTO workflow_executions` before adding columns.
...
Three idioms coexist because the editors evolved separately. Unifying them is a separate slice. For now: open the editor's existing tab-switch code, read how it renders, do the same thing. Pattern-mirror-before-pick.
...
**UI rule.** Defensive badges exist for all enum values — don't rely on the engine's current narrow write set. A future slice might start writing `skipped` (e.g. condition-gated steps), and dead UI paths are a hazard waiting to surface.
...
| `lib/internal_functions.js` | All 22 built-in functions |
...
*Last updated: April 2026. Covers YisraHook v1.2 (internal automation targets), plus cross-engine list envelope, contact-tying, ID-only sequences, capture mode, and monitoring-UI patterns. If the code diverges from this cookbook, the code is right.*
