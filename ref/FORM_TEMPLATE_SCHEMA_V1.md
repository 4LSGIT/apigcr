...
| `onSubmit.workflow` | optional | Same semantics as YCForm: form data spread as init vars, system fields win. **(X3.4)** `id` must be a positive integer; `initData`, if present, a JSON object. Mutually exclusive with `workflows` |
| `onSubmit.workflows` | optional | **(X3.4, Fred-ratified 2026-08-13)** The list form: 1–3 `{ id, initData? }` entries — EVERY entry fires on save (internal client AND external route), fire-and-forget, failures independent, each with the same assembly: field values as base, that ENTRY's `initData` overriding, system fields (and `_values` externally) always winning. The intended pair: the shared "Form Submission Notify" workflow as one entry (its `notify_to`/`labels`/`title_field` ride ITS `initData` — the workflow has no defaults of its own) plus a form-specific workflow beside it. Mutually exclusive with the legacy singular, which stays valid so stored definitions round-trip byte-identically |
...
#### 4.1.1 `chapter` — card-mode grouping (X6b, 2026-09-05)

```json
{ "chapter": "Vehicles", "rows": [ ... ] }
```

Optional **non-empty string**, standard sections, meaningful only under `layout: "card"` (harmless and ignored elsewhere). **Sticky-down:** a declaration starts a chapter that owns every following section until the next declaration. Sections before the first declaration group under an implicit `Start` pill (renderer warns; declare on `sections[0]` instead). Renders as the pill bar (current / started ring / done solid / error), chapter-scoped dot row, no counter text, and chapter headings on the review page. A definition with **zero** declarations renders byte-identically to pre-X6b — the emptiness is the feature gate.

Type-gated at publish (non-empty string or `badRequest`). **Never part of `fieldSignature`** — publishing a chapter edit cannot bump `schema_version`. In `SECTION_KEYS` (`extFormService.projectDefinition`) so it exists externally; old code stripping it degrades to the plain dot bar (the rollback story, by design). Live DBKQ carries 11 chapters as of 2026-09-05.
...
**Additional field keys (D4, 2026-09-06):**

| Key | Type | Notes |
|---|---|---|
| `prefix` / `suffix` | string | **(D4 A1)** Display-only input adornments (`"$"`, `"%"`, `"/mo"`). `number` and `text` only; ≤ 12 chars; rejected on every other type (silent-no-op refusal — the renderer styles exactly those two). Rendered as a `.yc-input-adorn` flex wrapper carrying the input's border/background/`:focus-within` ring with the adornment `<span class="yc-adorn">` inside it (the `.yc-tags-wrap` pattern) — the value pipeline (`collect()`, drafts, submit, PATCH) never sees the adornment. Valid in repeaters (same two types). **Not part of the §6 field-set signature** — publishing one never bumps `schema_version`. Empty string / null degrade to absent. |
...
| `number` | `input[type=number]` | `number` | optional `min`/`max`/`step`. **Since D4 (2026-09-06)** `min`/`max` are VALIDATED (§7: finite numbers, number-type only, `min <= max`) and ENFORCED: they still emit the native attributes, and the renderer additionally registers a `number` validation rule on EVERY number field — a strict plain-decimal format check (rejects `1e9` and pasted junk the native input tolerates) plus the DECLARED bounds. Never invents a bound: no declared `min` ⇒ a negative value is legal at validation. yc-forms also keystroke-filters every `type=number` input (delegated): `e`/`E`/`+` always dead, `-` dead unless the element's `min` attribute is negative. |
...
**Validation derivation:** any of `required`, `minLength`, `maxLength`, `pattern` (string, compiled with `new RegExp`), `patternMessage`, `email`, `mask` present → entry in YCForm `validation`. `mask` also sets `data-yc-mask` on the element. **`type: "number"` always derives a `number` rule** (format check unconditional; `min`/`max` bounds only when declared) — D4.

**On-blur validation (D4 A2):** yc-forms validates a field's FORMAT rules (`mask`, `email`, `pattern`, `number`) on `focusout`, painting/clearing only that field's own `.yc-error`. Completeness and cross-field rules (`required`, `requiredWhen`→`custom`, `minLength`/`maxLength`) stay gate-only — no nagging blank fields, no premature cross-field errors. Empty value clears any prior error. `focusout` (not `change`): bubbles for delegation, fires after the mask's blur reformat, and shares no event with card auto-advance or the conditional engine.
...
- `onSubmit.workflow` / `onSubmit.workflows` (X3.4): each entry is an object with a positive-integer-coercible `id` and an optional plain-object `initData`; `workflows` is a non-empty array of at most 3; the two keys are mutually exclusive.
...
| D4 | (delivered 2026-09-06) §4.3 `prefix`/`suffix` adornments (number/text, ≤12 chars, display-only, signature-excluded); §4.3/§7 `min`/`max` validated (finite, number-only, min<=max) and ENFORCED via the always-derived `number` rule (plain-decimal format + declared bounds); yc-forms: on-blur single-field FORMAT validation (`focusout`, mask/email/pattern/number only) + delegated keystroke filter on `type=number` (e/E/+ dead; `-` dead unless the min attribute declares negatives). validate() loop extracted to `_evaluateField` — blur reuses the gate's evaluator on a filtered rules subset, one validation system. DBKQ v1.2 (scripts/dbkq_v12_polish.js): min:0 on all 65 number fields, `$` prefix on the 63 dollar fields (household size + mileage excluded), fieldSignature unchanged. |
| X1–X3.2 | (delivered 2026-08) External-forms arc entries in THIS doc: `visibility` column + flip route (X1); `urlParam` field key + reserved-name list (X2); `external.badLink` (X1) and `external.postSubmit` `{message, edit, new}` (X3 — retroactively recorded here 2026-08-13; the amendment originally landed only in code + manual ch.15). Full arc record: `ref/EXTERNAL_FORMS_DESIGN.md`, `ref/archive/X2_DESIGN_AMENDMENTS.md`, scratch `external_arc`. |
...
| X3.4 | (2026-08-13, Fred-ratified) `onSubmit.workflows` (1–3 entries) beside the legacy singular — multiple workflows per submission, per-entry `initData`, dispatched by BOTH the internal client (yc-forms save step 5) and the external route with identical assembly; first-time shape validation of the legacy `workflow` (positive-int id). Builder: workflow group renders both shapes, converts to the list on "+ Add another"; new **Share links** block (direct `/f/`, branded `/p/form?f=`, sequence-merge variant, copy buttons). |