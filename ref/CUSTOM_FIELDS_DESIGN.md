...
**Status: DESIGN RATIFIED 2026-09-22 · S0 SHIPPED 2026-09-24 @ `e340356` · S1 SHIPPED 2026-09-24 @ `9bdc995` · S2 SHIPPED 2026-09-24 @ `45ac909` · S3 next**
...
- **Case merge vs custom (found S2, must close before S5):** `mergeCases` diffs `Object.keys(loser)` with `String()` — every `custom` compares equal as `[object Object]`, so the absorbed case's custom values are silently dropped (they survive only in the merge snapshot in `log_extra`). After S3 the same loop will also see the virtual `cf_` columns and try to `UPDATE` a generated column (MySQL 3105). Needs a per-key merge rule (e.g. fill absent keys from the loser via `JSON_MERGE_PATCH`, differing keys → `survivor_wins`) and generated columns in `SKIP`.
- **External egress of the bag (found S2):** `routes/api.intake.petition.js` `fetchContacts` returns `SELECT * FROM contacts` rows to the Pabbly caller when `get_contacts` is set — `custom: {}` today, whole values once fields hold data. Decide strip vs explicit column list before S4/S5.
- Settings editor has no option-retire toggle and does not show retired state (S2 made retire API-only; the service keeps a retired option retired across editor saves). Small UI follow-up.
