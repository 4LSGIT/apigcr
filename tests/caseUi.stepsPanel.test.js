...
function pipelinePayload(over = {}) {
  return Object.assign({
    ...
    current: { stage_id: 5, stage_key: 'docs', case_stage: 'Pending',
               status_label: 'Docs Requested', entered_at: '2026-08-28T19:12:57.000Z',
               entered_by: 6, source: 'manual', note: 'back on docs' },
    // The REAL live shape (TYL6KJN8, 2026-08-27): eight rows, repeats, and
    // three hops into the INTAKE template's contract_sent — a stage that has
    // no section in this payload's stage list and would vanish from a
    // stage-only view.
    history: [
      { stage_id: 18, stage_key: 'contract_sent', case_stage: 'Lead',
        status_label: 'Contract Sent — Awaiting Signature',
        entered_at: '2026-08-27T18:47:05.000Z', entered_by: 6, source: 'system', note: null },
      ...
      { stage_id: 18, stage_key: 'contract_sent', case_stage: 'Lead',
        status_label: 'Contract Sent — Awaiting Signature',
        entered_at: '2026-08-27T19:11:08.000Z', entered_by: 6, source: 'manual', note: null },
      { stage_id: 23, stage_key: 'retained', case_stage: 'Pending',
        status_label: 'Retained — Send Doc Request',
        entered_at: '2026-08-27T19:11:41.000Z', entered_by: 6, source: 'system', note: null },
      { stage_id: 5, stage_key: 'docs', case_stage: 'Pending',
        status_label: 'Docs Requested', entered_at: '2026-08-27T19:11:42.000Z',
        entered_by: 6, source: 'system', note: null },
      { stage_id: 18, stage_key: 'contract_sent', case_stage: 'Lead',
        status_label: 'Contract Sent — Awaiting Signature',
        entered_at: '2026-08-27T19:12:14.000Z', entered_by: 6, source: 'manual', note: null },
      { stage_id: 5, stage_key: 'docs', case_stage: 'Pending',
        status_label: 'Docs Requested', entered_at: '2026-08-28T19:12:57.000Z',
        entered_by: 6, source: 'manual', note: 'back on docs' },
    ],
...
describe('the Steps panel boots', () => {
  ...
  test('the CURRENT stage is marked ONCE, and only there', async () => {
    const { window } = await boot();
    const cur = $$(window, '.pw-stage-cur');
    expect(cur).toHaveLength(1);
    expect(txt(cur[0].querySelector('.pw-stage-name'))).toBe('Docs Requested');
    // "here now" is the single unambiguous marker — no second highlighted list.
    expect($$(window, '.pw-here').map(txt)).toEqual(['here now']);
  });

  test('entered-on dates come from the LATEST visit, not the first', async () => {
    // `docs` was entered twice: Aug 27 and Aug 28. A case that left and came
    // back is on its NEWEST visit; the first date describes a position the
    // case no longer holds.
    const { window } = await boot();
    const whens = $$(window, '.pw-steps-main .pw-stage .pw-stage-when').map(txt);
    expect(whens[0]).toBe('Entered Aug 27, 2026');    // retained (latest of 2)
    expect(whens[1]).toBe('Entered Aug 28, 2026');    // docs — the SECOND visit
    expect(whens[2]).toBe('');                        // filed — never entered
  });

  test('a stage entered more than once carries a xN marker (else the log is inexplicably longer)', async () => {
    const { window } = await boot();
    const stages = $$(window, '.pw-steps-main .pw-stage');
    expect(txt(stages[0].querySelector('.pw-repeat'))).toBe('×2');   // retained
    expect(txt(stages[1].querySelector('.pw-repeat'))).toBe('×3');   // docs
    expect(stages[2].querySelector('.pw-repeat')).toBeNull();        // filed
  });

  test('entered stages wear the log SNAPSHOT label; unentered ones the template label', async () => {
    // A stage renamed since the case passed through is NOT retro-renamed.
    const p = pipelinePayload();
    p.history = [{ stage_id: 23, stage_key: 'retained', case_stage: 'Pending',
                   status_label: 'Retained (old wording)',
                   entered_at: '2026-08-27T18:48:25.000Z', entered_by: 6,
                   source: 'manual', note: null }];
    p.current = { ...p.history[0] };
    const { window } = await boot({ pipeline: p, full: [] });
    const names = $$(window, '.pw-steps-main .pw-stage .pw-stage-name').map(txt);
    expect(names[0]).toBe('Retained (old wording)');   // snapshot
    expect(names[1]).toBe('Docs Requested');           // template (never entered)
  });

  test('the client-facing label rides alongside every stage', async () => {
    const { window } = await boot();
    expect($$(window, '.pw-steps-main .pw-stage .pw-client').map(txt))
      .toEqual(['Gathering your documents', 'Gathering your documents',
                'Gathering your documents']);
  });

  test('there is exactly ONE stage list — no separate timeline rendering the same stages', async () => {
    const { window } = await boot();
    // The regression this replaces: #pwTimeline and #pwSteps both listed the
    // template's stages, with the same labels, dates and highlight.
    expect(window.document.getElementById('pwTimeline')).toBeNull();
    expect(txt(window.document.getElementById('pwMsg'))).toBe('');
    const named = $$(window, '.pw-stage-name').map(txt);
    expect(named.filter(n => n === 'Docs Requested')).toHaveLength(1);
  });
  ...
  test('a payload with NO requirements anywhere renders NO steps view (zero-behaviour-change)', async () => {
    ...
    // The STAGE LIST still renders — it is the timeline. What is absent is the
    // requirement furniture: no rows, no legend, no "No requirements" filler.
    expect($$(window, '.pw-steps-main .pw-stage').length).toBeGreaterThan(0);
    expect($$(window, '.pw-req')).toHaveLength(0);
    expect(window.document.querySelector('.pw-legend')).toBeNull();
    expect(window.document.querySelector('.pw-stage-empty')).toBeNull();
  });
  ...
  test('no template → no steps view, and the panel still says why', async () => {
    ...
    expect(txt(window.document.getElementById('pwMsg')))
...
describe('off-ramp rail', () => {
  ...
  test('a case SITTING on an off-ramp is marked in the rail (position is lane-agnostic)', async () => {
    ...
    expect(here).toEqual(['Dismissed — here now']);
    // ...and no main-lane stage claims to be current at the same time.
    expect($$(window, '.pw-stage-cur')).toHaveLength(0);
  });

  test('the rail says what an off-ramp IS — reachable from anywhere, not next', async () => {
    const { window } = await boot();
    expect(txt(window.document.querySelector('.pw-rail-hint')))
      .toBe('Reachable from any stage');
  });
});
...
describe('requirement rows', () => {
  ...
  test('the owner badge appears ONLY when the owner is not the client', async () => {
    // Most requirements are the client's, so the badge carries information
    // only in the exceptions — printing "client" on every row is noise.
    const { window } = await boot();
    const rows = $$(window, '.pw-steps-main .pw-req');
    const client = rows.find(r => txt(r).includes('Upload requested documents'));
    const system = rows.find(r => txt(r).includes('341 meeting date set'));
    expect(client.querySelector('.pw-owner')).toBeNull();
    expect(txt(system.querySelector('.pw-owner'))).toBe('system');
  });

  test('a legend explains the glyphs, and each icon carries its own tooltip', async () => {
    const { window } = await boot();
    const legend = txt(window.document.querySelector('.pw-legend'));
    ['Done', 'Active now', 'Upcoming', 'Skipped', 'Not applicable']
      .forEach(w => expect(legend).toContain(w));
    const icon = window.document.querySelector('.pw-req-active .pw-req-icon');
    expect(icon.getAttribute('title')).toMatch(/Active now/);
  });

  test('a skipped row DROPS its hint — an instruction for work no longer asked for', async () => {
    const p = pipelinePayload({
      stages: [stage({ requirements: [
        req({ requirement_key: 'sk', internal_label: 'Skipped work', status: 'skipped',
              hint: 'Everything on your document request list' }),
      ] })],
      upcoming: [],
    });
    const { window } = await boot({ pipeline: p, full: [] });
    expect(txt(window.document.querySelector('.pw-req')))
      .not.toContain('Everything on your document request list');
  });

  test('the two dates in the panel are WORDED, because they mean different things', async () => {
    const p = pipelinePayload({
      stages: [S_RETAINED(), stage({ requirements: [
        req({ status: 'done', satisfied_at: '2026-03-04T15:00:00.000Z', detail: 'Signed' }),
      ] })],
      upcoming: [],
    });
    const { window } = await boot({ pipeline: p, full: [] });
    // Stage: when the case ARRIVED. Requirement: when the work was FINISHED.
    expect(txt(window.document.querySelector('.pw-stage-when'))).toMatch(/^Entered /);
    expect(txt(window.document.querySelector('.pw-req-sub'))).toMatch(/^Done Mar 4, 2026/);
  });
  ...
  test('a stage with no requirements says so rather than rendering an empty block', async () => {
    ...
    expect(txt(retained.querySelector('.pw-stage-empty')))
      .toBe('No requirements in this stage');
  });
});
...
describe('intake history', () => {
  test('a phase-`case` case fetches the full resolver and collapses the OFF-TEMPLATE rows', async () => {
    ...
    const block = window.document.querySelector('.pw-past-block');
    expect(block).not.toBeNull();
    // Labelled as PAST and placed under an "Earlier" heading — the first cut
    // hung this off the bottom of the stage list, where it read as something
    // that happens AFTER the last stage.
    expect(txt(block.querySelector('.pw-sec-head'))).toBe('Earlier');
    const details = block.querySelector('details');
    expect(details.hasAttribute('open')).toBe(false);          // collapsed
    expect(txt(details.querySelector('summary')))
      .toBe('Intake steps — from before this template (3)');
  ...
  test('the LOG is a second, separate collapsible — repeats and cross-template hops kept', async () => {
    const { window } = await boot();
    const summaries = $$(window, '.pw-past-block summary').map(txt);
    expect(summaries[1]).toBe('Stage history — every entry, newest first (8)');
    const rows = $$(window, '.pw-log');
    expect(rows).toHaveLength(8);                       // every entry, not 3 stages
    // Newest first.
    expect(txt(rows[0])).toContain('Aug 28, 2026');
    // A hop into a stage with NO section in this template is flagged, not
    // dropped — a stage-only view would lose it entirely.
    const offTpl = rows.filter(r => txt(r).includes('other template'));
    expect(offTpl).toHaveLength(3);
    offTpl.forEach(r => expect(txt(r)).toContain('Contract Sent'));
  });

  test('no history at all → no retrospective block', async () => {
    const p = pipelinePayload({ history: [], current: null });
    const { window } = await boot({ pipeline: p, full: [] });
    expect(window.document.querySelector('.pw-past-block')).toBeNull();
  });
  ...
  test('an INTAKE-phase case makes NO second GET — its payload already has everything', async () => {
    ...
    expect($$(window, '.pw-past-block summary').map(txt))
      .not.toContain(expect.stringContaining('Intake steps'));
  });
  ...
  test('the second GET failing costs the history section and NOTHING else', async () => {
    ...
    expect(errors).toEqual([]);
    expect($$(window, '.pw-past-block summary').map(txt))
      .toEqual([expect.stringContaining('Stage history')]);   // log survives
...
describe('projection (R2.5)', () => {
  ...
  test('present → a greyed "Typical next steps" section naming the target template', async () => {
    ...
    expect(txt(proj.querySelector('.pw-sec-head')))
      .toBe('After retention — Bankruptcy — Chapter 7 (typical, not yet this case)');
  ...
  test('a `generic` projection renders WITHOUT a template name', async () => {
    ...
    const { window } = await boot({ pipeline: p });
    expect(txt(window.document.querySelector('.pw-proj .pw-sec-head')))
      .toBe('After retention (typical, not yet this case)');
  });
