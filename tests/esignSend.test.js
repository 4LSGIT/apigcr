...
// ═════════════════════════════════════════════════════════════════════════════
// PHASE 2E — source storage, text fill, and resend-without-reattach
// ═════════════════════════════════════════════════════════════════════════════

describe('sendPipeline — source storage + text fill (2E)', () => {
  ...
  const drawnHex = (v) => Buffer.from(v, 'latin1').toString('hex').toUpperCase();
  ...
  function sendArgs(over = {}) {
    return {
      ...
      recipients: [{ name: 'John Smith', email: 'john@example.com', order: 1 }],
      createdBy: 1,
      pdfBuffer: buildPdf(1),
      ...over,
    };
  }

  test('textValues become ink; stored source is FILLED but UNSTAMPED', async () => {
    await svc.sendPipeline(makeDb(), sendArgs({
      placements: TEXT_PLACEMENTS,
      textValues: { debtor_name: 'John Q Smith' },
    }));

    // Provider buffer: value + footer.
    const sent = provider.sendForSignature.mock.calls[0][0].pdfBuffer;
    const sentStreams = inflateAllStreams(sent);
    expect(sentStreams).toContain(drawnHex('John Q Smith'));
    expect(countFooters(sent, TRACKING)).toBe(1);

    // Stored source: value, NO footer — both resend branches re-stamp.
    const [, storedId, storedBuf] = esignService.storeSourcePdf.mock.calls[0];
    expect(storedId).toBe(42);
    const storedStreams = inflateAllStreams(storedBuf);
    expect(storedStreams).toContain(drawnHex('John Q Smith'));
    expect(countFooters(storedBuf, TRACKING)).toBe(0);
  });

  test('no textValues → the original buffer is stored verbatim', async () => {
    const src = buildPdf(1);
    await svc.sendPipeline(makeDb(), sendArgs({ pdfBuffer: src }));
    const [, , storedBuf] = esignService.storeSourcePdf.mock.calls[0];
    expect(storedBuf).toBe(src); // identity — pdfFill's no-op path
  });

  test('source storage failing is BEST-EFFORT: send succeeds, event notes it', async () => {
    esignService.storeSourcePdf.mockRejectedValueOnce(new Error('disk on fire'));
    const out = await svc.sendPipeline(makeDb(), sendArgs());
    expect(out.row.status).toBe('sent');
    const evt = esignService.appendEvent.mock.calls
      .find((c) => c[2] && c[2].event === 'source_pdf_not_stored');
    expect(evt[2].payload.error).toContain('disk on fire');
  });

  test('provider failure → nothing stored (the row is still a draft)', async () => {
    provider.sendForSignature.mockRejectedValueOnce(Object.assign(new Error('zoho 500'), { code: 'ESIGN_PROVIDER' }));
    await expect(svc.sendPipeline(makeDb(), sendArgs())).rejects.toBeTruthy();
    expect(esignService.storeSourcePdf).not.toHaveBeenCalled();
  });
});

describe('resendPipeline — stored-source fallback (2E)', () => {
  beforeEach(() => {
    esignService.getById.mockResolvedValue(makeRow({ status: 'bounced', provider_id: 'ZS-OLD' }));
  });

  test('no buffer + stored source → same-row resend from storage, re-stamped once', async () => {
    const stored = buildPdf(1);
    esignService.getSourcePdf.mockResolvedValueOnce({ buffer: stored, size: stored.length });

    const out = await svc.resendPipeline(makeDb(), 42, { createdBy: 1 });

    expect(out.mode).toBe('same_row');
    const sent = provider.sendForSignature.mock.calls[0][0].pdfBuffer;
    expect(countFooters(sent, TRACKING)).toBe(1); // stamped exactly once — source was unstamped
    // and the source is re-upserted so it always equals what went out
    expect(esignService.storeSourcePdf).toHaveBeenCalledWith(expect.anything(), 42, stored);
  });

  test('an explicit upload WINS over the stored source', async () => {
    const replacement = buildPdf(2);
    await svc.resendPipeline(makeDb(), 42, { pdfBuffer: replacement, createdBy: 1 });
    expect(esignService.getSourcePdf).not.toHaveBeenCalled();
    expect(esignService.storeSourcePdf).toHaveBeenCalledWith(expect.anything(), 42, replacement);
  });

  test('no buffer + nothing stored → ESIGN_BAD_PDF, nothing sent', async () => {
    esignService.getSourcePdf.mockResolvedValueOnce(null);
    await expect(svc.resendPipeline(makeDb(), 42, { createdBy: 1 }))
      .rejects.toMatchObject({ code: 'ESIGN_BAD_PDF' });
    expect(provider.sendForSignature).not.toHaveBeenCalled();
  });

  test('a failing storage READ is its own error, not "never stored"', async () => {
    esignService.getSourcePdf.mockRejectedValueOnce(new Error('timeout'));
    await expect(svc.resendPipeline(makeDb(), 42, { createdBy: 1 }))
      .rejects.toMatchObject({ code: 'ESIGN_SOURCE_UNAVAILABLE' });
  });

  test('terminal duplicate also falls back to the stored source', async () => {
    const stored = buildPdf(1);
...
describe('routes/api.esign.actions.js', () => {
  ...
  test('the expected endpoints exist', () => {
    ...
    expect(sigs).toEqual([
      ...
      'GET /api/esign/:id(\\d+)/source',   // 2E: stored unsigned source download
