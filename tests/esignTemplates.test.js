...
describe('template CRUD', () => {
  test('createTemplate supplies BOTH JSON columns explicitly as strings (sql_mode landmine)', async () => {
    ...
    const prefillParam   = insert.params[3];
    const placementParam = insert.params[4];
...
describe('prefill resolvers', () => {
  ...
  test('firm.address is DELIBERATELY not a resolver (no source of truth exists)', () => {
    expect(WL.has('firm.address')).toBe(false);
  });
