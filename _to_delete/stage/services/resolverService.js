...
// ─────────────────────────────────────────────────────────────
// Expression scanner — PURE, exported (Phase 2E, esign)
// ─────────────────────────────────────────────────────────────

/**
 * Statically scan a template-expression string for its real-table references,
 * using the SAME scanner (findPlaceholders) and splitter (splitParts) the
 * resolver itself uses — so anything this reports is exactly what resolve()
 * would try to fetch, including refs hidden inside nested |default:{{...}}.
 *
 * Consumers (esignTemplateService) validate the result against
 * ALLOWED_TABLES / BLOCKED_COLUMNS + information_schema at SAVE time, so a
 * template can never store an expression that only fails at send time.
 *
 * PURE — no db, no refs, no side effects.
 *
 * @param {string} text
 * @returns {{ refs: Array<{table:string, column:string}>,
 *             triggerData: boolean,       // any {{trigger_data.*}} seen
 *             placeholderCount: number }} // top-level {{...}} found
 */
function scanExpressionRefs(text) {
  const refs = [];
  let triggerData = false;
  let placeholderCount = 0;

  const walk = (content, depth = 0) => {
    if (depth > 20) return;
    ...
    if (dotIdx !== -1) {
      const table  = head.slice(0, dotIdx).trim();
      const column = head.slice(dotIdx + 1).trim();
      if (table === 'trigger_data') triggerData = true;
      else refs.push({ table, column });
    }
  ...
  if (typeof text === 'string') {
    for (const { match } of findPlaceholders(text)) {
      placeholderCount += 1;
      walk(match.slice(2, -2));
    }
  }

  return { refs, triggerData, placeholderCount };
}
...
module.exports = { resolve, scanExpressionRefs, ALLOWED_TABLES, BLOCKED_COLUMNS };