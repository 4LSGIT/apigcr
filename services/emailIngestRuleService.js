 ...
 * clone can never fire on live mail — the caller (UI) edits then activates.
 *
 * Deliberately does NOT re-run the validator: the source rows already passed
 * validation on their own writes, and the copy is byte-identical config. A
 * source action whose target has since gone inactive copies verbatim — the
 * clone behaves exactly like its source until edited.
 *
