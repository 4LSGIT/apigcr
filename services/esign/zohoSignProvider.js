...
function neutralToZohoFields(placements, pageInfo) {
  ...
  // field_name is what Zoho RENDERS inside the box on the signing page (a
  // client saw 'Initial_2' — Fred, 2026-07-20), and it must be unique per
  // document. So: the author's label wins, de-duplicated with a numeric
  // suffix only on collision; unlabeled fields keep the old Type_N names.
  ...
  fields.forEach((f, i) => {
    ...
    const field = {
      ...
      field_label:      label || spec.field_type_name,
