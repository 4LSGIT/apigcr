 ...
 *                 label:  string,   // SIGNER fields, optional: what the SIGNER SEES on
 *                                   //   the signing page (Zoho renders field_name in the
 *                                   //   box; without this it shows 'Signature_4' etc.)
  ...
  fields.forEach((f, i) => {
    ...
    if (f.label != null) {
      ...
      if (typeof f.label !== 'string' || !f.label.trim() || f.label.length > 60) {
        throw inputError(
          `placements.fields[${i}].label must be a non-empty string of at most 60 characters when present`
        );
      }
    }
